...
(function verifyV16Selectors() {
	...
	console.log(
		"\n%c=== Frappe v16 DOM Selector Verification ===",
	...
	for (const [label, selector] of Object.entries(selectors)) {
		...
		if (elements.length > 0) {
			console.log(
				`%c✓ ${label}%c (${elements.length} found)`,
				"color: #16a34a;",
				"color: #64748b;",
				selector
			);
	...
	for (const [token, expected] of Object.entries(tokens)) {
		...
		if (expected === null) {
			...
			if (
				...
				console.log(
					`%c✗ ${token}%c expected "${expected}" but got "${value}"`,
					"color: #dc2626;",
					"color: #64748b;"
				);
				tokenFail++;
	...
	for (const sel of dupeSelectors) {
		...
		if (els.length > 0) {
			console.log(
				`%c✗ DUPLICATE SIDEBAR FOUND: ${sel}%c (${els.length} elements)`,
	...
	allNavbars.forEach((el, i) => {
		console.log(
			`  [${i}] <${el.tagName.toLowerCase()}> class="${el.className}" id="${
				el.id || "none"
			}"`
		);
	});
	...
	const scripts = performance
		...
		.filter((r) => r.name.includes("construction"));
	scripts.forEach((s) => {
		const isOld =
			s.name.includes("theme_patch") ||
			s.name.includes("modern_theme_loader") ||
			s.name.includes("theme_system_2025");
		console.log(
			`%c${isOld ? "⚠" : "✓"} ${s.name.split("/").pop()}%c`,
			isOld ? "color: #dc2626; font-weight: bold;" : "color: #16a34a;",
			"color: #64748b;"
		);
	});
	...
	if (sidebar) {
		...
		if (hasInlineWidth || hasInlinePosition) {
			console.log(
				"%c⚠ Sidebar has inline styles that may override CSS!",
	...
	console.log("\n%c════════════════════════════════════════", "color: #94a3b8;");
	console.log(
		`%cDOM Selectors: ${found}/${total} found, ${missing} not in DOM (navigate to relevant page to see more)`,
		found === total
			? "color: #16a34a; font-weight: bold;"
			: "color: #f59e0b; font-weight: bold;"
	);
	console.log(
		`%cCSS Tokens: ${tokenPass} pass, ${tokenFail} fail`,
		tokenFail === 0
			? "color: #16a34a; font-weight: bold;"
			: "color: #dc2626; font-weight: bold;"
	);
	console.log(
		`%cNote: Many selectors only appear on specific pages (forms, lists, workspaces). Navigate to each page type and re-run this script for full coverage.`,
