...
OUTPUT = ROOT / "docs/translation/stage6_w605_setup_rows_2026-09-24.csv"
MAX_LEN = 120
EXPECTED_SHA = "e2d672c4a1b479f9cd52c017f76a3c8a8b1b24dced24ed4b5f5c6d8113900a09"
...
def is_technical_token(source):
    return (
        ...
        and (any(char.isdigit() for char in source) or "-" in source or source.isupper())
    )
...
def main():
    ...
    for row in read_rows(LEDGER):
        ...
        if not source or area_of(first_location(row)) != "setup" or source in seen:
            continue
        seen.add(source)
        raw.append(row)
    assert len(raw) == 483
    ...
    assert len(scope) == 469
    assert len({row["msgid"] for row in scope}) == 469
