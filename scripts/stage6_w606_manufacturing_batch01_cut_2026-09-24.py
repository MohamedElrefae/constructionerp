"""Deterministically cut the proposal-only W6-6 Manufacturing batch 01."""
...
OUTPUT = ROOT / "docs/translation/stage6_w606_manufacturing_batch01_rows_2026-09-24.csv"
CAP = 250
MAX_LEN = 120
...
def main() -> None:
    ...
    assert raw_count == 481, f"Expected 481 ledger rows, got {raw_count}"
    assert len(raw_rows) == 480, f"Expected 480 unique source keys, got {len(raw_rows)}"
    ...
    assert len(overlap) == 1, f"Expected one already-covered key, got {len(overlap)}"
    assert len(fresh) == 450, f"Expected 450 fresh rows, got {len(fresh)}"
    batch = fresh[:CAP]
    ...
    print(f"fresh_total={len(fresh)} batch01={len(batch)} remaining_not_proposed={len(fresh) - len(batch)}")
    print(
        "batch01_prefilled="
