...
| 2026-05-25 | P2/P6 | Pricing `has_stages` UI regression fix | Passed | Added numeric-safe Pricing comparison, BOQ Item Stage form/list links, migrated, built assets, and targeted BOQ suite passed: 18 tests |
| 2026-05-25 | P6-01 | Project filter selector crash fix | Passed | Added Frappe saved-filter group label compatibility patch for translated UI; `bench build --app construction` and `bench --site v16.localhost clear-cache` passed |
| 2026-05-25 | P6-01 | Arabic translation UX + apply fix | Passed | Added Translation list tools (Arabic-only and missing-Arabic prepare/filter), safer translation update override to avoid deleting context rows, and dropdown sizing fix for language filter; `bench build --app construction` and cache clear passed |
