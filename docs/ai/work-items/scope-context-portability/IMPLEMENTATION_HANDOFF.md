# Implementation Handoff — LangGraph Workflow Orchestration and Offline Pilot

| Field | Value |
|---|---|
| Work item | `scope-context-portability` |
| Document | IMPLEMENTATION_HANDOFF.md — synchronized working copy of CANONICAL_PLAN.md |
| Revision | **r5 (working handoff)** — includes the five pre-Phase-0 contract corrections |
| Date | 2026-09-11 |
...
> **Purpose.** This working handoff mirrors the numbered contracts of `CANONICAL_PLAN.md` r5 and is self-contained. The canonical plan is authoritative if the copies ever diverge. Keep both documents synchronized in the same change. Their coherence sign-off does not itself authorize Phase 0, commits, or ERP operations.
...
| 6 | Stage 4 export authority: the governed manifest `construction/data/localization/stage4_export_manifest.json` records export `account_catalog_20260910_121018.json`, SHA-256 `206ecfae…`, recorded 2026-09-10T12:10:18Z. Builder evidence `evidence/stage-4-export.md` describes the earlier 2026-09-09 export (SHA `d9699243…`). | The manifest is the current authority; the hash difference is historical, not an error. Adoption records the manifest reference and never regenerates the export. |
...
*Revision r5 (working handoff), 2026-09-11. The five bounded corrections are resolved in the document before Phase 0. Implementation evidence is still to be produced under the phase gates. No Phase 0 execution, commits, ERP reruns or data mutation are performed by this revision.*
