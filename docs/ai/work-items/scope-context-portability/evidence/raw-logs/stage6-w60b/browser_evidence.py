#!/usr/bin/env python3
"""W6-0b batch-1 Arabic browser evidence (headless, ar Desk session)."""
...
OUT = Path("/tmp/opencode/stage6w60b")
PASSWORD = "ct-w60b-evidence-1"
PROBE = json.loads((OUT / "probe_keys.json").read_text(encoding="utf-8"))
PLACEHOLDER_SRC = "No currency fields in {0}"

results = {
    "batch": "stage6-w60b-batch1",
...
with sync_playwright() as p:
    ...
    probes = {k: v for k, v in PROBE.items() if k != PLACEHOLDER_SRC}
    ...
    ph_got = page.evaluate(
        """(src)=>{try{return __(src,['Currency']);}catch(e){return 'ERR:'+e.message;}}""",
        PLACEHOLDER_SRC,
    )
    ph_exp = PROBE[PLACEHOLDER_SRC].replace("{0}", "Currency")
    add("translate-placeholder", ph_got == ph_exp, {"expected": ph_exp, "got": ph_got})
    ...
    page.screenshot(path=str(OUT / "browser-ar-desk.png"), full_page=False)
