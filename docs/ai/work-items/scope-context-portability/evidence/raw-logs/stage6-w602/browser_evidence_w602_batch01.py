#!/usr/bin/env python3
"""W6-2 batch-01 Arabic browser evidence (headless, ar Desk session)."""
...
PROBES = {
    ...
    "% of materials billed against this Sales Order": "% o نسبة المواد المفوترة ضد أمر البيع هذا",
    "% of materials delivered against this Sales Order": "% o نسبة المواد المسلَّمة ضد أمر البيع هذا",
...
PLACEHOLDER_SRC = "Added Supplier Role to User {0}."
PLACEHOLDER_EXP = "تمت إضافة دور المورد للمستخدم User."

results = {
    "batch": "stage6-w602-batch01",
...
with sync_playwright() as p:
    ...
    ph_got = page.evaluate(
        """(src)=>{try{return __(src,['User']);}catch(e){return 'ERR:'+e.message;}}""",
        PLACEHOLDER_SRC,
    )
    add(
    ...
    page.screenshot(path=str(OUT / "browser-ar-desk-w602-batch01.png"), full_page=False)
...
(OUT / "browser_evidence_w602_batch01.json").write_text(
