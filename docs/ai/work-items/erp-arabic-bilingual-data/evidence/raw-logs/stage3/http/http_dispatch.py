...
preq = requests.Request("POST", BASE + "/api/method/login", data={"usr": "Administrator", "pwd": "ct-browser-evidence-1"}).prepare()
...
r2_req = requests.Request(
    "POST",
    BASE + "/api/method/erpnext.accounts.doctype.account.account.update_account_number",
    data={"name": ACC, "account_name": "CT HTTP Probe", "account_number": "CT-HTTP-2", "reason": "Live HTTP dispatch evidence"},
).prepare()
