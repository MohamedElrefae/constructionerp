...
| 2026-09-22 | Owner partial production authorization: data source = live master/ledger on **`http://v16.localhost:8000`**; production **site** named as **`http://v16.localhost:8000`**; **rollout window not yet named** — full production approval still incomplete | Plan §18 steps 2–3; AI-R `stage8-production-ai-r-verify-0c8057f.md` |
| 2026-09-22 | Stage 8 isolated backup/restore rehearsal drill executed and **CLOSED** on authorized test backup set → `v16rehearsal.localhost` (never over `v16.localhost`, never production): restore completeness (GL=4, posted JVs, AR 6,000, draft invoice untouched, 30/15 files, encryption key injected, `ar`/Egypt, `REHEARSAL-SITE-MARKER`); routing isolation root-caused (`dns_multitenant: true` + pinned `bench --site … serve --port 8002`); hardened UAT preflight **PASS**; Playwright ar-Desk browser drill **PASS** (AR 5 rows / GL 14 rows bilingual). Stage-2 evidence re-pinned same day on pre-commit HEAD `80787af` (freshness re-collected to live CSV `4b3f0c9f…`, manifest re-recorded, ten envelopes + index atomic set, evidence-inclusive gate exit-0); committed as `0433497`. Owner then repaired a real Stage-8 preflight blocker on that commit (`UnboundLocalError` on connection-failure cleanup path) — fixed as `0c8057f` with offline coverage (cleanup sentinel, `--port=8002` routing, POST login; preflight 5/5, report 9/9, lint clean, bootstrap gate pass). **Production rollout NOT fully approved.** Only valid next sequence: (1) **AI-R verifies exact commit `0c8057f`** (not `0433497`) — **✅ PASS 2026-09-22** (`stage8-production-ai-r-verify-0c8057f.md`), (2) real production master/ledger data — **✅ 2026-09-22: live data at `http://v16.localhost:8000`**, (3) explicit authorization of the named production site + **rollout window** — site **`http://v16.localhost:8000` ✅** / window **⛔ still required**. Until the window is supplied: production untouched, W6-0b deferred | `stage8-restore-rehearsal-drill-2026-09-22.md`; `stage8-production-ai-r-verify-0c8057f.md`; `scripts/uat_preflight.py` POST/`--port` fixes + `0c8057f` cleanup sentinel; Stage-2 `raw-logs/stage2/` on `80787af`; assembler 89→91; commits `0433497`, `0c8057f` |

## 18. Hold state (owner directive, 2026-09-21; corrected status 2026-09-22)

**Stage-8 restore rehearsal: CLOSED.** **Production rollout: NOT fully approved — window still missing.**

Do **not** start W6-0b or any production mutation. Keep `develop` clean and
synced. Production stays untouched until **all three** of the following, in
order:
...
2. ~~**Real production master/ledger data provided**~~ ✅ **Provided
   2026-09-22 (owner):** use live master/ledger already on
   **`http://v16.localhost:8000`** as both the data source and the
   in-scope site (owner: “use http://v16.localhost:8000 as site and data”).
   Note: this host has historically been the project’s authorized test
   site; the owner is now naming it as the production target for this
   rollout.
3. **Explicit authorization** naming the production site **and the rollout
   window.**
   - Production site: **`http://v16.localhost:8000`** — ✅ named by owner
     2026-09-22.
   - Rollout window (start/end or “immediate / continuous”) — ⛔ **STILL
     REQUIRED.** No production mutation begins until the owner supplies it.
