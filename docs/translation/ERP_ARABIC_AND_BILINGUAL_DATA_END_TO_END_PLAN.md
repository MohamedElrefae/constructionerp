...
1. **AI-R verifies exact commit `0c8057f`** (not `0433497` — that revision
   ...
   coverage for cleanup + `--port=8002` routing + POST login). Migration
   set, data hashes, tests, rollback evidence, and evidence bundle must be
   verified against `0c8057f`.
