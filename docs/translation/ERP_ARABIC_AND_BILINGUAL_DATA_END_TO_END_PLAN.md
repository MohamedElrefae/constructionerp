...
| 2026-09-22 | Owner correction: **`http://v16.localhost:8000` is not production** — classification **non-production test**, `production_mutation_authorized: false` (test fixtures; rehearsal/UAT only). Therefore sequence **steps 2 and 3a are NOT satisfied** and **no rollout window is named**. Hold kept: no production mutation, W6-0b, catalog changes, or DB changes. Real production site identifier + confirmed production master/ledger data + explicit rollout window will arrive **together** when available | Plan §18; supersedes the partial site/data note earlier the same day |
...
Do **not** start W6-0b, production mutation, catalog changes, or DB changes.
Keep `develop` clean and synced. Production stays untouched until **all three**
...
Completed under prior hold (authorized test-site / isolated-rehearsal scope
only, no production mutation):
