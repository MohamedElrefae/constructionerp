...
| 5 | Wave 1 masters (Item, Customer, Supplier, …) | 🟡 **Readiness recorded; data migration not started** — Arabic fields installed (`item_name_ar`, `customer_name_in_arabic`, `supplier_name_in_arabic`), 0 values populated; bilingual search verified live on all three `schema_installed` doctypes; site inventory shows real master data is minimal (vendor `_Test*` fixtures excluded by governance). Owner gate: defer to Stage 8 with real masters (recommended) vs pilot on candidate masters now | 2026-09-20 | `docs/ai/work-items/scope-context-portability/evidence/stage5-wave1-readiness-2026-09-20.md` |
