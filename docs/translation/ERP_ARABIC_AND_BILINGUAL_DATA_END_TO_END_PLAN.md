...
| 4 | Visible bilingual display of the migrated Arabic names | 🔴 **Not started** — Stage 4 stores the values; no tree/list/Link/form/report renderer shipped (Stage 3 D2/D3 + C2/E1, reports Stage 7). `account_name_ar` currently has `in_list_view=0`, `in_standard_filter=0`, no Property Setters, and `account.js` is absent | 2026-09-20 | — |
| 3 | Bilingual registry, display service, account form section, localized tree, search/tree adapters | 🔴 **Not started** — required before any user-visible Arabic account name outside the raw tree field | — | — |
