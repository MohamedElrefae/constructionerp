...
async function navigateToList(page, doctype) {
	await page
		.goto(`${BASE_URL}/app/${doctype.replace(/\s+/g, "-").toLowerCase()}`, {
			waitUntil: "networkidle",
			timeout: 20000,
		})
		.catch(() => {});
...
async function runAllTests() {
	...
	await t("Verify baseline quantity revisions exist", async (p) => {
		...
		if (revisions.length < 2)
			return `Expected >=2 baseline revisions, got ${revisions.length}`;
	...
	await t("Verify original_qty and current_revised_qty on items", async (p) => {
		for (const itemName of [state.item1Name, state.item2Name]) {
			...
			if (r.message.current_revised_qty == null)
				return `current_revised_qty is NULL for ${itemName}`;
		}
		return true;
	});
	...
	await t("Verify total_revised_value equals total_contract_value", async (p) => {
		...
		if (Math.abs(trv - tcv) > 0.01)
			return `total_revised_value (${trv}) != total_contract_value (${tcv})`;
	...
	await t("Submit VO (Draft -> Submitted)", async (p) => {
		...
		if (!statusR.message || statusR.message.status !== "Submitted")
			return "Failed to set status to Submitted";
	...
	await t("Approve VO by Engineer", async (p) => {
		...
		if (!statusR.message || statusR.message.status !== "Approved by Engineer")
			return "Failed to approve by Engineer";
		await navigateToForm(p, "Variation Order", state.voIncreaseName);
		const text = await p.textContent("body");
	...
	await t("Approve VO by Client", async (p) => {
		...
		if (!statusR3.message || statusR3.message.status !== "Approved by Client")
			return "Failed to approve by Client";
		await navigateToForm(p, "Variation Order", state.voIncreaseName);
	...
	await t("Verify current_revised_qty matches revised_qty", async (p) => {
		...
		if (Math.abs(r.message.current_revised_qty - 126) > 0.01)
			return `current_revised_qty=${r.message.current_revised_qty}, expected 126`;
	...
	await t("Create and approve Quantity Decrease VO", async (p) => {
		...
		if (!check.message || check.message.status !== "Approved by Client")
			return "Decrease VO not approved";
		...
		return `current_revised_qty = ${
			itemR.message ? itemR.message.current_revised_qty : "null"
		}, expected 90`;
	});
	...
	await t("Verify omitted item hidden from transaction selectors", async (p) => {
		...
		if (!omittedVisible)
			return "Omitted item should be visible without exclude_zero_revised filter";
		return true;
	});
	...
	await t("Create New Variation Item VO", async (p) => {
		...
		if (!line.created_boq_item)
			return "created_boq_item is empty. New item may not have been created.";
	...
	await t("Verify variation item: is_variation_item=1, original_qty=0", async (p) => {
		...
		if (Math.abs(item.original_qty - 0) > 0.01)
			return `original_qty = ${item.original_qty}, expected 0`;
		if (Math.abs(item.current_revised_qty - 15) > 0.01)
			return `current_revised_qty = ${item.current_revised_qty}, expected 15`;
	...
	await t("Verify VOs display correctly in UI list", async (p) => {
		...
		const text = await p.textContent("body");
		const hasVOs = [
			state.voIncreaseName,
			state.voDecreaseName,
			state.voOmissionName,
			state.voNewItemName,
	...
	await t("Re-save approved VO - no duplicate revisions (P0-4)", async (p) => {
		...
		if (countAfter !== countBefore)
			return `Revisions increased: ${countBefore} -> ${countAfter}`;
