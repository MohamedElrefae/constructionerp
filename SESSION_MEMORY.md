...
**Accomplished:** Deterministic remainder cut from W6-2 raw 337 (first-location buying/selling) after batch-01 closed. Exact scope CSV `stage6_w602_batch02_rows_2026-09-23.csv` — **48 rows**, sha256 `9a096273f2f7ef05a403edffbb4b75604b73ce7f854382c8fd570f5c329e44ce` (48 selling: 1 pre-filled `Address` + 47 unfilled; 17 placeholders; all ≤120; no HTML). Overlap **0** vs released catalog 2275, batch 01 (270), all prior W6-0b scope/payload/released files (26), technical exclusions (21), a1/a2 dedup (302). **Scope accounting corrected after owner review:** raw **337 = 268 (b01 in raw) + 48 (b02) + 21 residual**; residual **21 = 5 HTML + 15 `len>120` + 1 already-released** (mutually exclusive under cut priority; HTML∩`len>120`=3 counted once under HTML); batch-01 **file** size 270 = 268 raw + 2 synthetic keys (`' Address'`, `'Could not find path for '`) outside the 337 — prior prose wrongly wrote `270+48+20=338`. CSV/checksum **unchanged**. Proposal md filed; **no quorum, no import, no catalog change, no evidence re-pin**.
**Decisions:** Awaiting owner approval of that exact 48-row CSV only (accounting corrected first). Evidence re-pin deferred to the next catalog-changing cycle. Production Stage 8 still gated.
**Open issues:** None new. Untracked `v16.localhost/` at app root is cwd-relative Frappe/ipython log debris (4 files, 116K) — left unstaged/untracked (owner: can stay as-is).
**Next:** Owner mark-up on corrected batch-02 proposal → only then governed cycle on `v16.localhost`. No production work authorized.
