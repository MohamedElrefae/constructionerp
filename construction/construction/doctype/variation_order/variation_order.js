	...
	boq_header(frm) {
		...
		frappe.db.get_value("BOQ Header", frm.doc.boq_header, "status").then((r) => {
			...
			if (status && status !== "Locked" && !frm.is_new()) {
				frappe.show_alert({
					message: __("BOQ Header status is {0}; VO can only be saved against Locked BOQs.", [status]),
					indicator: "orange",
				});
			}
		});
...
frappe.ui.form.on("VO Line", {
	...
	boq_structure(frm, cdt, cdn) {
		...
		if (row.line_type !== "New Item" && row.boq_structure) {
			...
			frappe.db.get_value("BOQ Item", { structure: row.boq_structure }, "name").then((r) => {
	...
	boq_item(frm, cdt, cdn) {
		...
		if (row.boq_item) {
			...
			frappe.db.get_value("BOQ Item", row.boq_item, 
				["structure", "current_revised_qty", "quantity", "contract_unit_price", "title", "unit"]
			).then((r) => {
				const item = r?.message || {};
				if (item.structure && !row.boq_structure) {
					frappe.model.set_value(cdt, cdn, "boq_structure", item.structure);
				}
				frappe.model.set_value(cdt, cdn, "previous_qty", item.current_revised_qty || 0);
				frappe.model.set_value(cdt, cdn, "contract_qty", item.quantity || 0);
				frappe.model.set_value(cdt, cdn, "contract_unit_price", item.contract_unit_price || 0);
...
function apply_variation_order_workflow_buttons(frm) {
	...
	["Submit to Engineer", "Approve by Engineer", "Approve by Client", "Reject"].forEach((label) => {
	...
	if (status === "Draft") {
		frm.add_custom_button(__("Submit to Engineer"), () => transition_variation_order(frm, "Submitted"), __("Workflow"));
		frm.add_custom_button(__("Reject"), () => transition_variation_order(frm, "Rejected"), __("Workflow"));
	} else if (status === "Submitted") {
		frm.add_custom_button(__("Approve by Engineer"), () => transition_variation_order(frm, "Approved by Engineer"), __("Workflow"));
		frm.add_custom_button(__("Reject"), () => transition_variation_order(frm, "Rejected"), __("Workflow"));
	} else if (status === "Approved by Engineer") {
		frm.add_custom_button(__("Approve by Client"), () => prompt_client_approval(frm), __("Workflow"));
		frm.add_custom_button(__("Reject"), () => transition_variation_order(frm, "Rejected"), __("Workflow"));
	} else if (status === "Approved by Client") {
		frm.add_custom_button(__("Create Material Request"), () => create_mr_for_vo(frm), __("Create"));
	}
}
		...
		fields: [
			{
				...
				reqd: 1,
				description: __("A signed PDF from the client is required before final approval."),
			},
			{
...
function render_variation_order_indicators(frm) {
	...
	const headline = {
		...
		"Approved by Client": __("Client approved — variation items and revised quantities are now in effect."),
...
function apply_vo_line_readonly_state(frm, cdt, cdn) {
	...
	grid_row.toggle_editable("revised_unit_price", is_editable && (is_new_item || row.rate_change_triggered));
