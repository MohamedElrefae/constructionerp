...
class BOQItem(Document):
	...
	def fetch_cost_item_rate(self):
		if self.cost_item:
			self.est_unit_cost = frappe.db.get_value("Cost Item", self.cost_item, "total_direct_cost") or 0
		else:
	...
	def update_header_total(self):
		if not self.boq_header:
			return
		total = frappe.db.sql(
			"SELECT COALESCE(SUM(line_total), 0) FROM `tabBOQ Item` WHERE boq_header = %s", self.boq_header
		)
