...
class BOQItem(Document):
    PRICING_EDITABLE = frozenset({
        "est_unit_price", "contract_unit_price", "factor",
        "cost_item", "item_type",
        ...
        if status == "Pricing":
            ...
            for field in self.meta.get_valid_columns():
                if field in self.PRICING_EDITABLE:
                    continue
                if field in ("name", "modified", "modified_by", "creation",
                ...
                if str(old_val or "") != str(new_val or ""):
                    frappe.throw(
                        _("Cannot modify field '{0}': BOQ is in Pricing status. "
                          "Only pricing-related fields can be edited.").format(field)
                    )
    ...
    def validate_input_guards(self):
        ...
        non_negative_fields = [
            "quantity", "factor", "est_unit_cost",
            "est_unit_price", "contract_unit_price",
        ]
        for field in non_negative_fields:
            val = self.get(field) or 0
            if val < 0:
                frappe.throw(
                    _("Field '{0}' must be non-negative. Got: {1}").format(field, val)
                )
        ...
        for field in pct_fields:
            ...
            if val < 0 or val > 100:
                frappe.throw(
                    _("Field '{0}' must be between 0 and 100. Got: {1}").format(field, val)
                )
    ...
    def validate_output_guards(self):
        ...
        output_fields = [
            "overhead_amount", "profit_amount", "calculated_sell_price",
            "est_line_total", "line_total",
        ]
        for field in output_fields:
            val = self.get(field) or 0
            if val < 0:
                frappe.throw(
                    _("Computed field '{0}' must be non-negative. Got: {1}").format(field, val)
                )
