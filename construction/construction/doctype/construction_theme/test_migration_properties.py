...
# Fix the __path__ of the cached 'construction' module if it was imported as a namespace package
if "construction" in sys.modules:
    correct_path = os.path.join(base_dir, "construction")
    sys.modules["construction"].__path__ = [correct_path]

import frappe
from hypothesis import HealthCheck, given, settings
from hypothesis import strategies as st

from construction.patches.v6_0.set_default_new_theme_fields import (
    _darken_hex,
    _is_dark_theme,
    _lighten_hex,
)
...
def hex_color_strategy():
    ...
    three_digit = st.text(alphabet="0123456789ABCDEFabcdef", min_size=3, max_size=3).map(lambda x: "#" + x)
    six_digit = st.text(alphabet="0123456789ABCDEFabcdef", min_size=6, max_size=6).map(lambda x: "#" + x)
...
def theme_name_strategy():
    ...
    return st.text(
        alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 -_", min_size=1, max_size=50
...
class TestMigrationProperties(unittest.TestCase):
    ...
    @given(
        ...
        border_color=hex_color_strategy(),
    )
    ...
    def test_migration_computes_correct_defaults(
        ...
        try:
            ...
            expected_primary_btn_hover_bg = (
                _lighten_hex(accent_primary, 0.1) if is_dark else _darken_hex(accent_primary, 0.1)
            )
            ...
            expected_secondary_btn_hover_bg = (
                _lighten_hex(surface_bg, 0.1) if is_dark else _darken_hex(surface_bg, 0.1)
            )
            ...
            theme.flags.ignore_update_modified = True
            theme.save()
    ...
    def test_migration_preserves_original_fields(
        ...
        try:
            ...
            theme.primary_btn_hover_bg = (
                _lighten_hex(accent_primary_val, 0.1) if is_dark else _darken_hex(accent_primary_val, 0.1)
            )
    ...
    def test_migration_skips_pre_populated_fields(
        ...
        body_bg,
        text_primary,
        pre_populated_primary_btn_bg,
        pre_populated_table_header_bg,
    ):
        """
        Property 14: Migration Skips Pre-Populated Fields
