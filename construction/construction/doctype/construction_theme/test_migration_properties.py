...
def hex_color_strategy():
	...
	three_digit = st.just("#").then(
		lambda _: st.text(alphabet="0123456789ABCDEFabcdef", min_size=3, max_size=3).map(lambda x: "#" + x)
	)

	six_digit = st.just("#").then(
		lambda _: st.text(alphabet="0123456789ABCDEFabcdef", min_size=6, max_size=6).map(lambda x: "#" + x)
	)
