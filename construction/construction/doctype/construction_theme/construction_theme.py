...
class ConstructionTheme(Document):
	...
	def _validate_unique_defaults(self):
		...
		if self.is_default_light:
			existing = frappe.db.exists(
				"Construction Theme", {"is_default_light": 1, "is_active": 1, "name": ("!=", self.name)}
			)
			if existing:
				frappe.throw(
					_(
		...
		if self.is_default_dark:
			existing = frappe.db.exists(
				"Construction Theme", {"is_default_dark": 1, "is_active": 1, "name": ("!=", self.name)}
			)
			if existing:
				frappe.throw(
					_(
	...
	def _validate_login_page_fields(self):
		...
		has_any_login_config = any(
			[
				...
				self.get("login_heading_text_color"),
				self.get("login_tab_bg_color"),
				bg_color,
				bg_image,
			]
		)
	...
	def _validate_login_page_bg_image_publicity(self):
		...
		file_doc = frappe.db.get_value("File", {"file_url": bg_image}, ["name", "is_private"], as_dict=True)
...
"""
	...
	def get_system_default():
		...
		return frappe.db.get_value(
			"Construction Theme", {"is_system_theme": 1, "is_active": 1}, ["name", "theme_name"], as_dict=True
		)
	...
	def get_default_for_mode(mode):
		...
		return frappe.db.get_value(
			"Construction Theme", {mode_field: 1, "is_active": 1}, ["name", "theme_name"], as_dict=True
		)
