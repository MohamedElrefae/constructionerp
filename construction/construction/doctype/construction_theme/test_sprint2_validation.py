...
def invalid_hex_color_strategy():
    ...
    return st.one_of(
        st.text(min_size=1, max_size=20).filter(
            lambda x: not re.match(r"^#[0-9A-Fa-f]{3}$|^#[0-9A-Fa-f]{6}$", x)
        ),
        st.just(""),
        st.just(None),
        st.just("#"),
        ...
        if len(title) > 30:
            ...
            try:
                ...
                raise AssertionError(
                    f"Expected validation error for title length {len(title)}, but insert succeeded"
                )
...
class TestComponentStylesheetStructure(unittest.TestCase):
    ...
    def test_component_stylesheet_exists(self):
        ...
        self.assertTrue(
            os.path.exists(self.stylesheet_path), f"Component stylesheet not found at {self.stylesheet_path}"
        )
    ...
    def test_ct_theme_rules_exist(self):
        ...
        for css_class in required_classes:
            self.assertIn(
                f".{css_class}", content, f"CSS class .{css_class} not found in component stylesheet"
            )
    ...
    def test_display_none_used_for_hiding(self):
        ...
        for rule in hide_rules:
            self.assertTrue(
                "display: none" in rule or "display:none" in rule, f"Rule {rule} does not use display:none"
            )
    ...
    def test_css_variables_referenced(self):
        ...
        self.assertGreater(len(var_matches), 0, "No CSS variables (--ct-*) found in component stylesheet")
...
class TestLoginValidationEdgeCases(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        test_themes = frappe.db.get_list(
            "Construction Theme", filters={"theme_name": ["like", "Test Login%"]}
        )
    ...
    def test_login_bg_type_mismatch_image_no_image(self):
        ...
        error_msg = str(context.exception).lower()
        self.assertIn("image", error_msg, f"Error message should mention image: {context.exception}")
