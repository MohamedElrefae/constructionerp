...
REQUIRED_FIELDS = {
    ...
    "theme_type": st.sampled_from([
        "Construction Light", "Construction Dark", "Custom Light", "Custom Dark"
    ]),
...
OPTIONAL_COLOR_FIELDS = [
    "accent_primary_hover", "accent_secondary", "text_secondary",
    "border_color", "success_color", "warning_color", "error_color",
    "primary_btn_bg", "primary_btn_text", "primary_btn_hover_bg",
    "primary_btn_hover_text", "secondary_btn_bg", "secondary_btn_text",
    "secondary_btn_hover_bg", "secondary_btn_hover_text",
    "table_header_bg", "table_header_text", "table_body_bg",
    "table_body_text", "number_card_bg", "number_card_border",
    "number_card_text", "input_bg", "input_border", "input_text",
    "input_label_color", "navbar_text_color", "footer_bg", "footer_text",
]
...
def _build_mock_theme(fields: dict):
    ...
    class MockTheme:
        ...
        def generate_css_variables(self):
            ...
            if pbg and not pbhbg: pbhbg = self._compute_hover_color(pbg)
            sbg = self.get('secondary_btn_bg')
            sbhbg = self.get('secondary_btn_hover_bg')
            if sbg and not sbhbg: sbhbg = self._compute_hover_color(sbg)
            for fn, vn in self.FIELD_VAR_MAP.items():
                if fn == 'primary_btn_hover_bg' and pbhbg:
                    ...
                    val = self.get(fn)
                    if val: variables.append(f"{vn}:{val}")
...
@given(
    ...
    accent_primary=hex_color, navbar_bg=hex_color, sidebar_bg=hex_color,
    surface_bg=hex_color, body_bg=hex_color, text_primary=hex_color,
...
def test_property_1_css_variable_round_trip(
    theme_name, theme_type, accent_primary, navbar_bg, sidebar_bg,
    surface_bg, body_bg, text_primary, optional_colors
):
    """Property 1: Round-trip — generate then parse back yields equivalent map."""
    fields = {
        "theme_name": theme_name, "theme_type": theme_type,
        "accent_primary": accent_primary, "navbar_bg": navbar_bg,
        "sidebar_bg": sidebar_bg, "surface_bg": surface_bg,
        "body_bg": body_bg, "text_primary": text_primary,
        **optional_colors,
    }
...
def test_property_2_css_output_format(
    ...
    fields = {
        "theme_name": theme_name, "theme_type": "Custom Light",
        "accent_primary": accent_primary, "navbar_bg": navbar_bg,
        "sidebar_bg": sidebar_bg, "surface_bg": surface_bg,
        "body_bg": body_bg, "text_primary": text_primary,
    }
...
def test_property_5_auto_computed_hover(btn_bg, body_bg):
    ...
    fields = {
        "theme_name": "test", "theme_type": "Custom Light",
        "accent_primary": "#2076FF", "navbar_bg": "#ffffff",
        "sidebar_bg": "#f0f0f0", "surface_bg": "#ffffff",
        "body_bg": body_bg, "text_primary": "#000000",
