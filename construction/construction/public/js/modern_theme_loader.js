...
(function () {
	...
	console.log(
		"%c[Modern Theme] LOADER v2.0 - 20250416",
	...
	const ModernThemeLoader = {
		...
		applyTheme: function (forcedMode) {
			...
			console.log(
				`[Modern Theme] applyTheme called with forcedMode=${forcedMode}, currentDomMode=${currentDomMode}, sending=${modeToSend}`
			);
			...
			frappe.call({
				...
				callback: function (r) {
					...
					if (r.message) {
						...
						console.log(
							`[Modern Theme] BEFORE override - theme_name: ${theme_name}, mode: ${mode}, forcedMode: ${forcedMode}, modeToSend: ${modeToSend}`
						);
						...
						if (forcedMode) {
							if (mode !== forcedMode) {
								console.log(
									`[Modern Theme] OVERRIDE: API returned mode "${mode}" but forcedMode is "${forcedMode}", using forcedMode`
								);
								mode = forcedMode;
								source = "dom_override";
							}
							...
							console.log(
								`[Modern Theme] OVERRIDE: API returned mode "${mode}" but we sent "${modeToSend}", using modeToSend`
							);
							mode = modeToSend;
						...
						console.log(
							`[Modern Theme] AFTER override - Final applying: ${theme_name} (${mode}, source: ${source})`
						);
						...
						console.log(
							`[Modern Theme] VERIFICATION: Final document data-theme is: ${finalMode}`
						);
					} else {
				...
				error: function (err) {
					...
					if (forcedMode) {
						console.log(
							`[Modern Theme] Applying forced mode ${forcedMode} despite API error`
						);
			...
			const observer = new MutationObserver(function (mutations) {
				mutations.forEach(function (mutation) {
					if (mutation.attributeName === "data-theme") {
						...
						console.log(
							`[Modern Theme] MutationObserver: data-theme changed from "${oldMode}" to "${newMode}"`
						);

						if (newMode && newMode !== oldMode) {
							console.log(
								"[Modern Theme] Mode change confirmed, calling applyTheme with:",
								newMode
							);
							...
							console.log(
								"[Modern Theme] Mode unchanged or invalid, skipping applyTheme"
							);
						}
					}
				});
		...
		persistModePreference: function (mode) {
			frappe.call({
				...
				error: function (err) {
					console.log(
						"[Modern Theme] Mode save error (non-critical):",
						err.message || err
					);
				},
			});
                            ...
                            <a class="nav-link" href="#"
                               ...
                               onclick="return false;">
                                <span id="theme-mode-dot" style="
                                    ...
                                    border-radius: 50%;
                                    background: ${
										self.currentMode === "dark" ? "#60a5fa" : "#fbbf24"
									};
                                    box-shadow: 0 0 8px ${
										self.currentMode === "dark" ? "#60a5fa" : "#fbbf24"
									};
	...
	if (frappe.ui && frappe.ui.ThemeSwitcher) {
		frappe.ui.ThemeSwitcher = class CustomThemeSwitcher extends frappe.ui.ThemeSwitcher {
				...
				return new Promise((resolve) => {
					this.themes = [
						...
						{ name: "dark", label: "🌙 Dark", info: "Timeless Night Theme" },
						{
							...
							info: "Modern light theme optimized for construction workflows",
						},
						{
							name: "construction_dark",
							label: "🏗️ Construction Dark",
							info: "Modern dark theme optimized for construction workflows",
						},
						{
							name: "automatic",
							label: "⚡ Automatic",
			...
			switch_theme(theme_name) {
				...
				super.switch_theme(
					theme_name === "construction_light"
						? "light"
						: theme_name === "construction_dark"
						? "dark"
						: theme_name
				);
			...
			render() {
				try {
					...
					console.log(
						"[Modern Theme] Parent ThemeSwitcher render error (non-critical):",
						e.message
					);
				}
			...
			_applyVisualIndicators() {
				...
				if (!this.$wrapper || !this.$wrapper.length) {
					console.log(
						"[Modern Theme] ThemeSwitcher $wrapper not ready after defer, relying on CSS indicators"
					);
					return;
				}
				...
				this.$wrapper.find(".theme-item").each(function () {
					...
					const isSelected =
						...
						(selectedTheme === "construction_light" &&
							itemTheme === "light" &&
							currentTheme !== "Standard") ||
						(selectedTheme === "construction_dark" &&
							itemTheme === "dark" &&
							currentTheme !== "Standard");
					...
					if (isSelected) {
						...
						if (!$item.find(".selected-indicator").length) {
							$item
								.find(".theme-label")
								.prepend('<span class="selected-indicator">✓ </span>');
						}
					}
				});
			}
