...
class TestBOQProperties(unittest.TestCase):
	...
	def test_property2_wbs_round_trip(self):
		...
		for code in test_codes:
			...
			self.assertEqual(code, rebuilt, f"Round-trip failed for code: {code} -> {rebuilt}")

			# Also test that segments are correct
			expected_segments = code.split(".")
			self.assertEqual(segments, expected_segments, f"Parsing failed for code: {code}")
	...
	def test_property6_line_total_calculation(self):
		...
		for qty, price, factor, expected in test_cases:
			...
			self.assertAlmostEqual(
				line_total, expected, places=2, msg=f"Failed for qty={qty}, price={price}, factor={factor}"
			)
	...
	def test_property7_header_rollup_total(self):
		...
		self.assertAlmostEqual(
			self.boq_header.total_contract_value,
			total_expected,
			places=2,
	...
	def test_property9_auto_create_boq_item(self):
		...
		self.assertTrue(item_exists, "BOQ Item should be auto-created for leaf nodes")

		# Get the auto-created item
		item = frappe.get_doc("BOQ Item", {"structure": leaf_structure.name})

		# Verify the item is linked correctly
		self.assertEqual(item.structure, leaf_structure.name, "BOQ Item should be linked to the structure")
		self.assertEqual(item.boq_header, self.boq_header.name, "BOQ Item should have the same boq_header")
		...
		self.assertFalse(item_exists, "BOQ Item should NOT be auto-created for section nodes")
	...
	def test_property10_cascade_delete_boq_item(self):
		...
		item_exists = frappe.db.exists("BOQ Item", {"structure": leaf_structure.name})
		self.assertFalse(item_exists, "BOQ Item should be cascade-deleted when leaf node is deleted")
...
if __name__ == "__main__":
	...
	def test_property16_migration_data_preservation(self):
		...
		migration_header = frappe.get_doc(
			{"doctype": "BOQ Header", "title": "Migration Test BOQ", "status": "Draft", "boq_type": "Tender"}
		)
