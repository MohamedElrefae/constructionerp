    ...
    it('config length equals descriptors length and defaults are preserved', function () {
        fc.assert(
            fc.property(arbDescriptorArray(), function (descriptors) {
                ...
                // Same length
                assert.equal(config.length, descriptors.length,
                ...
                config.forEach(function (col) {
                    ...
                    assert.ok(desc, 'Config entry field_key "' + col.field_key + '" must exist in descriptors');
                    assert.equal(col.visible, desc.default_visible,
                        'visible must match default_visible for ' + col.field_key);
                    assert.equal(col.width, desc.default_width,
                        'width must match default_width for ' + col.field_key);
                    assert.equal(col.sort_order, desc.default_sort_order,
                        'sort_order must match default_sort_order for ' + col.field_key);
                    assert.equal(col.label, desc.label,
    ...
    it('config field_keys are exactly the descriptor field_keys (no extras, no missing)', function () {
        fc.assert(
            fc.property(arbDescriptorArray(), function (descriptors) {
                ...
                var configKeys = config.map(function (c) { return c.field_key; }).sort();
                var descKeys = descriptors.map(function (d) { return d.field_key; }).sort();

                assert.deepEqual(configKeys, descKeys,
        ...
        fc.assert(
            fc.property(arbDescriptorArray(), function (descriptors) {
                ...
                for (var i = 1; i < config.length; i++) {
                    assert.ok(config[i].sort_order >= config[i - 1].sort_order,
        ...
        fc.assert(
            fc.property(
                ...
                function (input) {
                    ...
                    // Build lookups
                    var savedMap = {};
                    saved.forEach(function (s) { savedMap[s.field_key] = s; });

                    var descMap = {};
                    descriptors.forEach(function (d) { descMap[d.field_key] = d; });
                    ...
                    config.forEach(function (col) {
                        ...
                        if (savedEntry) {
                            ...
                            assert.equal(col.width, savedEntry.width,
                                'width must use saved value for ' + col.field_key);
                            assert.equal(col.visible, savedEntry.visible,
                                'visible must use saved value for ' + col.field_key);
                            assert.equal(col.sort_order, savedEntry.sort_order,
        ...
        fc.assert(
            fc.property(
                ...
                function (descriptors) {
                    ...
                    var saved = [
                        { field_key: 'nonexistent_1', label: 'X', width: 50, visible: false, sort_order: 99 },
                        { field_key: 'nonexistent_2', label: 'Y', width: 25, visible: true, sort_order: 88 }
                    ];
                    ...
                    config.forEach(function (col) {
                        ...
                        assert.equal(col.visible, desc.default_visible,
                            'visible must be default when no matching saved setting for ' + col.field_key);
                        assert.equal(col.sort_order, desc.default_sort_order,
                            'sort_order must be default when no matching saved setting for ' + col.field_key);
                    });
                }
            ),
            { numRuns: 100 }
        ...
        fc.assert(
            fc.property(
                ...
                function (args) {
                    ...
                    assert.equal(configAfter.length, configBefore.length,
...
describe('Property 4: Width Validation', function () {
        ...
        fc.assert(
            fc.property(
                ...
                function (args) {
                    ...
                    assert.equal(col.width, newWidth,
        ...
        fc.assert(
            fc.property(
                ...
                function (args) {
                    ...
                    assert.equal(col.width, widthBefore,
                        'Column width must remain unchanged at ' + widthBefore + ' after rejected width ' + invalidWidth);
                }
            ),
            { numRuns: 100 }
        );
        ...
        fc.assert(
            fc.property(
                ...
                function (args) {
                    ...
                    assert.equal(result, false,
                        'set_width must return false for non-number type');
    ...
    it('set_width with valid value does not affect other columns', function () {
        fc.assert(
            fc.property(
                arbDescriptorArray().filter(function (d) { return d.length >= 2; }).chain(function (descriptors) {
                    var idx = fc.integer({ min: 0, max: descriptors.length - 1 });
                    var validWidth = fc.integer({ min: 1, max: 100 });
                    return fc.tuple(fc.constant(descriptors), idx, validWidth);
                }),
                function (args) {
                    ...
                    configAfter.forEach(function (col) {
                        ...
                        if (col.field_key !== targetKey) {
                            ...
                            assert.equal(col.sort_order, before.sort_order,
                                'Non-target column ' + col.field_key + ' sort_order must be unchanged');
                        }
                    });
                }
            ),
        ...
        fc.assert(
            fc.property(
                ...
                function (input) {
                    ...
                    assert.equal(mgr.get_width_total(), expectedTotal,
        ...
        fc.assert(
            fc.property(
                ...
                function (descriptors) {
                    ...
                    assert.equal(mgr.get_width_total(), 0,
        ...
        fc.assert(
            fc.property(
                ...
                function (args) {
                    ...
                    var keysBefore = mgr.get_config().map(function (c) { return c.field_key; }).sort();
                    ...
                    var keysAfter = configAfter.map(function (c) { return c.field_key; }).sort();
                    ...
                    for (var i = 0; i < configAfter.length; i++) {
                        assert.equal(configAfter[i].sort_order, i,
                            'sort_order must be consecutive 0-based; expected ' + i +
                            ' at position ' + i + ' but got ' + configAfter[i].sort_order);
                    }

                    // 3. Moved column is at the target position
                    assert.equal(configAfter[toIndex].field_key, movedKey,
                        'Moved column "' + movedKey + '" must be at target position ' + toIndex +
                        ' but found "' + configAfter[toIndex].field_key + '"');
                }
            ),
            { numRuns: 100 }
        );
        ...
        fc.assert(
            fc.property(
                ...
                function (args) {
                    ...
                    for (var i = 0; i < configAfter.length; i++) {
                        assert.equal(configAfter[i].field_key, configBefore[i].field_key,
                            'field_key at position ' + i + ' must be unchanged');
                        assert.equal(configAfter[i].sort_order, configBefore[i].sort_order,
                            'sort_order at position ' + i + ' must be unchanged');
                        assert.equal(configAfter[i].width, configBefore[i].width,
                            'width at position ' + i + ' must be unchanged');
                        assert.equal(configAfter[i].visible, configBefore[i].visible,
        ...
        fc.assert(
            fc.property(
                ...
                function (args) {
                    ...
                    // Same count
                    assert.equal(configAfter.length, descriptors.length,
        ...
        fc.assert(
            fc.property(
                ...
                function (input) {
                    ...
                    // Same length
                    assert.equal(resetConfig.length, freshConfig.length,
                        'Reset config length must equal fresh config length');

                    // Each column must be identical
                    for (var i = 0; i < resetConfig.length; i++) {
                        assert.equal(resetConfig[i].field_key, freshConfig[i].field_key,
                            'field_key at position ' + i + ' must match after reset');
                        assert.equal(resetConfig[i].label, freshConfig[i].label,
                            'label at position ' + i + ' must match after reset');
                        assert.equal(resetConfig[i].width, freshConfig[i].width,
                            'width at position ' + i + ' must match after reset');
                        assert.equal(resetConfig[i].visible, freshConfig[i].visible,
                            'visible at position ' + i + ' must match after reset');
                        assert.equal(resetConfig[i].sort_order, freshConfig[i].sort_order,
        ...
        fc.assert(
            fc.property(
                ...
                function (input) {
                    ...
                    assert.equal(mgr.get_width_total(), freshTotal,
        ...
        fc.assert(
            fc.property(
                ...
                function (input) {
                    ...
                    assert.equal(mgr.to_json(), freshJson,
        ...
        fc.assert(
            fc.property(
                ...
                function (input) {
                    ...
                    assert.equal(parsed.length, config.length,
                        'Parsed config length must equal get_config() length');

                    for (var i = 0; i < config.length; i++) {
                        assert.equal(parsed[i].field_key, config[i].field_key,
                            'field_key at position ' + i + ' must match');
                        assert.equal(parsed[i].visible, config[i].visible,
                            'visible at position ' + i + ' must match');
                        assert.equal(parsed[i].width, config[i].width,
                            'width at position ' + i + ' must match');
                        assert.equal(parsed[i].sort_order, config[i].sort_order,
                            'sort_order at position ' + i + ' must match');
                        assert.equal(parsed[i].label, config[i].label,
        ...
        fc.assert(
            fc.property(
                ...
                function (input) {
                    ...
                    assert.equal(config2.length, config1.length,
                        'Reconstructed config length must match original');

                    for (var i = 0; i < config1.length; i++) {
                        assert.equal(config2[i].field_key, config1[i].field_key,
                            'field_key at position ' + i + ' must match after round-trip');
                        assert.equal(config2[i].visible, config1[i].visible,
                            'visible at position ' + i + ' must match after round-trip');
                        assert.equal(config2[i].width, config1[i].width,
                            'width at position ' + i + ' must match after round-trip');
                        assert.equal(config2[i].sort_order, config1[i].sort_order,
...
describe('PreviewPanel Rendering', function () {
    ...
    it('renders column widths in <th> style attributes', function () {
        ...
        assert.ok(container.innerHTML.indexOf('width:15%') !== -1,
            'Should contain width:15% for first column');
        assert.ok(container.innerHTML.indexOf('width:40%') !== -1,
    ...
    it('renders exactly 3 <tr> in tbody when 3 sample rows provided', function () {
        ...
        var columns = [
            { field_key: 'id', label: 'ID', width: 100, visible: true, sort_order: 0 }
        ];
    ...
    it('renders placeholder row with column labels when no sample data provided', function () {
        ...
        assert.ok(tbodyMatch[1].indexOf('WBS Code') !== -1,
            'Placeholder row should contain "WBS Code" label');
        assert.ok(tbodyMatch[1].indexOf('Title') !== -1,
    ...
    it('renders placeholder row when sample_data is undefined', function () {
        ...
        var tbodyTrCount = countMatches(tbodyMatch[1], /<tr>/g);
        assert.equal(tbodyTrCount, 1, 'Should render 1 placeholder row when sample_data is undefined');

        assert.ok(tbodyMatch[1].indexOf('Quantity') !== -1,
    ...
    it('renders empty <td> for columns whose field_key is not in sample data rows', function () {
        ...
        var columns = [
            { field_key: 'wbs_code', label: 'WBS Code', width: 30, visible: true, sort_order: 0 },
            { field_key: 'title', label: 'Title', width: 40, visible: true, sort_order: 1 },
            { field_key: 'missing_field', label: 'Missing', width: 30, visible: true, sort_order: 2 }
        ];
        ...
        assert.ok(tdMatches[1].indexOf('Foundation') !== -1, 'Second cell should contain "Foundation"');
        assert.equal(tdMatches[2], '<td></td>', 'Third cell should be empty for missing field_key');
    });
