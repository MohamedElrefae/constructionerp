...
class TestConstructionThemeSchema(unittest.TestCase):
	...
	def test_all_11_tabs_exist(self):
		...
		for expected_tab in expected_tabs:
			self.assertIn(expected_tab, tab_labels, f"Tab '{expected_tab}' not found in DocType")
	...
	def test_general_tab_fields(self):
		...
		for field_name in required_fields:
			self.assertIn(field_name, self.fields, f"Field '{field_name}' not found in General tab")
	...
	def test_login_page_tab_fields(self):
		...
		for field_name in required_fields:
			self.assertIn(field_name, self.fields, f"Field '{field_name}' not found in Login Page tab")
	...
	def test_buttons_tab_fields(self):
		...
		for field_name in required_fields:
			self.assertIn(field_name, self.fields, f"Field '{field_name}' not found in Buttons tab")
	...
	def test_tables_tab_fields(self):
		...
		for field_name in required_fields:
			self.assertIn(field_name, self.fields, f"Field '{field_name}' not found in Tables tab")
	...
	def test_widgets_tab_fields(self):
		...
		for field_name in required_fields:
			self.assertIn(field_name, self.fields, f"Field '{field_name}' not found in Widgets tab")
	...
	def test_input_fields_tab_fields(self):
		...
		for field_name in required_fields:
			self.assertIn(field_name, self.fields, f"Field '{field_name}' not found in Input Fields tab")
	...
	def test_navbar_tab_fields(self):
		...
		for field_name in required_fields:
			self.assertIn(field_name, self.fields, f"Field '{field_name}' not found in Navbar tab")
	...
	def test_footer_tab_fields(self):
		...
		for field_name in required_fields:
			self.assertIn(field_name, self.fields, f"Field '{field_name}' not found in Footer tab")
	...
	def test_semantic_colors_tab_fields(self):
		...
		for field_name in required_fields:
			self.assertIn(field_name, self.fields, f"Field '{field_name}' not found in Semantic Colors tab")
	...
	def test_preview_tab_fields(self):
		...
		for field_name in required_fields:
			self.assertIn(field_name, self.fields, f"Field '{field_name}' not found in Preview tab")
	...
	def test_advanced_tab_fields(self):
		...
		for field_name in required_fields:
			self.assertIn(field_name, self.fields, f"Field '{field_name}' not found in Advanced tab")
	...
	def test_color_field_types(self):
		...
		for field_name in color_fields:
			self.assertIn(field_name, self.fields, f"Color field '{field_name}' not found")
			field = self.fields[field_name]
			self.assertEqual(
				field.fieldtype, "Color", f"Field '{field_name}' should be Color type, got {field.fieldtype}"
			)
	...
	def test_check_field_types(self):
		...
		for field_name in check_fields:
			self.assertIn(field_name, self.fields, f"Check field '{field_name}' not found")
			field = self.fields[field_name]
			self.assertEqual(
				field.fieldtype, "Check", f"Field '{field_name}' should be Check type, got {field.fieldtype}"
			)
	...
	def test_data_field_types(self):
		...
		for field_name in data_fields:
			self.assertIn(field_name, self.fields, f"Data field '{field_name}' not found")
			field = self.fields[field_name]
			self.assertEqual(
				field.fieldtype, "Data", f"Field '{field_name}' should be Data type, got {field.fieldtype}"
			)
	...
	def test_select_field_types(self):
		...
		for field_name in select_fields:
			self.assertIn(field_name, self.fields, f"Select field '{field_name}' not found")
			field = self.fields[field_name]
			self.assertEqual(
				field.fieldtype,
				"Select",
	...
	def test_theme_name_is_unique(self):
		...
		self.assertEqual(field.unique, 1, "theme_name should be marked as unique")

	def test_theme_name_is_required(self):
		"""Verify theme_name field is required."""
		field = self.fields.get("theme_name")
		self.assertIsNotNone(field, "theme_name field not found")
		self.assertEqual(field.reqd, 1, "theme_name should be required")
	...
	def test_theme_type_is_required(self):
		...
		self.assertEqual(field.reqd, 1, "theme_type should be required")
	...
	def test_navbar_bg_is_required(self):
		...
		self.assertEqual(field.reqd, 1, "navbar_bg should be required")
	...
	def test_custom_css_field_type(self):
		...
		self.assertEqual(field.fieldtype, "Code", f"custom_css should be Code type, got {field.fieldtype}")
	...
	def test_preview_colors_field_type(self):
		...
		self.assertEqual(
			field.fieldtype, "JSON", f"preview_colors should be JSON type, got {field.fieldtype}"
		)
	...
	def test_contrast_ratio_field_type(self):
		...
		self.assertEqual(
			field.fieldtype, "Float", f"contrast_ratio should be Float type, got {field.fieldtype}"
		)
	...
	def test_description_field_type(self):
		...
		self.assertEqual(
			field.fieldtype, "Small Text", f"description should be Small Text type, got {field.fieldtype}"
		)
	...
	def test_all_original_15_fields_preserved(self):
		...
		for field_name in original_fields:
			self.assertIn(field_name, self.fields, f"Original field '{field_name}' not found or was renamed")
	...
	def test_login_page_bg_type_has_options(self):
		...
		self.assertIn("Solid Color", options, "login_page_bg_type should have 'Solid Color' option")
		self.assertIn("Background Image", options, "login_page_bg_type should have 'Background Image' option")
	...
	def test_login_box_position_has_options(self):
		...
		options = field.options.split("\n") if field.options else []
		self.assertIn("Default", options, "login_box_position should have 'Default' option")
		self.assertIn("Left", options, "login_box_position should have 'Left' option")
		self.assertIn("Right", options, "login_box_position should have 'Right' option")
	...
	def test_theme_type_has_options(self):
		...
		options = field.options.split("\n") if field.options else []
		self.assertGreater(len(options), 0, "theme_type should have options defined")
