...
def main():
    ...
    env["gate-tests-standalone.txt"] = (ts("standalone.start"), [m.group(0), ok.group(0), "TESTS_SHA256: " + art["TESTS_SHA256"]], ts("standalone.finish"))

    body = []
    total = 0
    for (mod,) in MODULES:
        txt = clean("mod_%s.out" % mod)
        mm = re.search(r"^Ran (\d+) tests in ([\d.]+s)$", txt, re.M)
        assert mm and re.search(r"^OK$", txt, re.M), mod
        total += int(mm.group(1))
        body.append("%s :: %s OK" % (mod, mm.group(0)))
    body.append("AGGREGATE total=%d failed=0" % total)
    ...
    env["final-dryrun.txt"] = (ts("dryrun.start"), [line, "CSV_SHA256: " + art["CSV_SHA256"], "RUNTIME_DIGEST: " + fresh["runtime_digest"], "DECISIONS_SHA256: " + art["DECISIONS_SHA256"]], ts("dryrun.finish"))

    json_text = (ROOT / ARTIFACT_PATHS["FRESHNESS_SHA256"]).read_text(encoding="utf-8")
    body = ["ARTIFACT: construction/data/localization/freshness_evidence.json",
            "ARTIFACT_SHA256: " + art["FRESHNESS_SHA256"],
            "FRESHNESS_SHA256: " + art["FRESHNESS_SHA256"],
            "--- JSON START ---"] + json_text.rstrip("\n").splitlines() + ["--- JSON END ---"]
    ...
    env["full-gate.txt"] = (ts("fullgate.start"), [gate_line, "CHECKER_SHA256: " + art["CHECKER_SHA256"], "PO_SHA256: " + art["PO_SHA256"], "CSV_SHA256: " + art["CSV_SHA256"], "MANIFEST_SHA256: " + art["MANIFEST_SHA256"], "NOTE: bootstrap run for envelope generation; CI and verification never pass this flag"], ts("fullgate.finish"))

    lint = clean("lints.raw")
    env["lints-diffcheck.txt"] = (ts("lints.start"), [next(l for l in lint.splitlines() if l.startswith("PASS: no scope-dimension")), "Translation write lint PASSED", "DIFFCHECK_CLEAN", "SCOPELINT_SHA256: " + art["SCOPELINT_SHA256"], "TRANSLATIONLINT_SHA256: " + art["TRANSLATIONLINT_SHA256"]], ts("lints.finish"))

    mraw = clean("merkle.raw")
    governed = []
    for mark in ("MANIFEST: ", "MANIFEST_SHA256: ", "INVENTORY_MANIFEST_SHA256: ", "MERKLE_ROOT: ", "MERKLE_ROWS: ", "SQL_SHA256: "):
    ...
    env["scoped-gate.txt"] = (ts("scoped.start"), [next(l for l in sraw.splitlines() if l.startswith("SKIP ")), next(l for l in sraw.splitlines() if l.startswith("checked=")), "PO_SHA256: " + art["PO_SHA256"]], ts("scoped.finish"))
    ...
    env["vendor-audit.txt"] = (ts("vendor.start"), ["errors=0", "BASELINE_SHA256: " + art["BASELINE_SHA256"], "FRAPPE_PO_SHA256: " + art["FRAPPE_PO_SHA256"], "ERPNext_PO_SHA256: " + art["ERPNext_PO_SHA256"]], ts("vendor.finish"))

    NO_REORDER = {"freshness-envelope.txt", "merkle.txt"}
    for fname in EVIDENCE_FILES:
        start, body, finish = env[fname]
        if fname in NO_REORDER:
            lines = [EXPECTED_COMMANDS[fname], "STARTED_UTC: " + start] + body + ["EXIT_CODE: 0", "FINISHED_UTC: " + finish]
        else:
            marks = [l for l in body if re.match(r"^[A-Za-z_]+_(SHA256|DIGEST): ", l) or l.startswith("NOTE: ")]
            results = [l for l in body if l not in marks]
            lines = [EXPECTED_COMMANDS[fname], "STARTED_UTC: " + start] + results + ["EXIT_CODE: 0", "FINISHED_UTC: " + finish] + marks
        lines.append("ENVELOPE_LINES: %d" % (len(lines) + 1))
        (EV / fname).write_text("\n".join(lines) + "\n", encoding="utf-8")

    head = subprocess.run(["git", "-C", str(ROOT), "rev-parse", "HEAD"], capture_output=True, text=True).stdout.strip()
    lines = ["INDEX_VERSION: 1",
    ...
    lines += ["EXIT_CODE: 0", "FINISHED_UTC: " + finish, "CANDIDATE_HEAD: " + head,
              "CANDIDATE_ROOT: " + str(ROOT.resolve()), "ARTIFACTS:"]
