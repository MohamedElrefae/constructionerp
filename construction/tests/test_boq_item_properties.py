...
class TestCostBuildupFormulas:
    ...
    def test_overhead_formula(self, est_unit_cost, overhead_pct, profit_pct):
        overhead, profit, sell_price, _ = run_cost_buildup(
            est_unit_cost, overhead_pct, profit_pct, 1, 1
        )
    ...
    @given(est_unit_cost=currency, overhead_pct=pct, profit_pct=pct,
           quantity=currency, factor=factor_st)
    @settings(max_examples=100)
    def test_est_line_total_formula(self, est_unit_cost, overhead_pct, profit_pct,
                                     quantity, factor):
        _, _, _, est_line_total = run_cost_buildup(
...
class TestOutputNonNegativity:
    ...
    @given(est_unit_cost=currency, overhead_pct=pct, profit_pct=pct,
           quantity=currency, factor=factor_st, contract_unit_price=currency)
    @settings(max_examples=100)
    def test_all_outputs_non_negative(self, est_unit_cost, overhead_pct, profit_pct,
                                       quantity, factor, contract_unit_price):
...
class TestIdempotence:
    ...
    @given(est_unit_cost=currency, overhead_pct=pct, profit_pct=pct,
           quantity=currency, factor=factor_st, contract_unit_price=currency)
    @settings(max_examples=100)
    def test_idempotent_calculations(self, est_unit_cost, overhead_pct, profit_pct,
                                      quantity, factor, contract_unit_price):
        # First run
        oh1, pr1, sp1, elt1 = run_cost_buildup(
            est_unit_cost, overhead_pct, profit_pct, quantity, factor
        )
        lt1 = run_line_total(quantity, contract_unit_price, factor)

        # Second run (identical inputs)
        oh2, pr2, sp2, elt2 = run_cost_buildup(
            est_unit_cost, overhead_pct, profit_pct, quantity, factor
        )
...
class TestHeaderRollup:
    ...
    def test_rollup_sums(self, items):
        ...
        recomputed_tcv = sum(
            run_line_total(i["quantity"], i["contract_unit_price"], i["factor"])
            for i in items
        )
        recomputed_tev = sum(
            run_cost_buildup(i["est_unit_cost"], i["overhead_pct"], i["profit_pct"],
                             i["quantity"], i["factor"])[3]
            for i in items
        )
        recomputed_tbc = sum(
            i["quantity"] * i["est_unit_cost"] * i["factor"]
            for i in items
        )
        ...
        assert close_enough(total_contract_value, recomputed_tcv), \
            f"tcv: {total_contract_value} vs {recomputed_tcv}"
        assert close_enough(total_est_line_total, recomputed_tev), \
            f"tev: {total_est_line_total} vs {recomputed_tev}"
        assert close_enough(total_budgeted_cost, recomputed_tbc), \
