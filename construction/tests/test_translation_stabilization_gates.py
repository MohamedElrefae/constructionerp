...
class TestTranslationStabilizationGates(FrappeTestCase):
    ...
    def test_b01_unique_digest_enforced(self):
        idx = frappe.db.sql(
            "SHOW INDEX FROM `tabTranslation` WHERE Column_name='ct_key_digest' AND Non_unique=0 AND Key_name='ct_translation_key_digest'",
            as_dict=True,
        )
    ...
    def test_b04_quorum_enforced(self):
        ...
        with tempfile.NamedTemporaryFile(
            ...
            w.writerow(
                {
                    "language": "ar",
                    "source_text": f"Quorum {uuid4().hex}",
                    "context": "",
                    "ct_app": "construction",
                    "translated_text": "قيمة",
                    "domain": "test",
                    "release_status": "Released",
                    "release_version": "1.0",
                    "a1_reviewer": "A1",
                    "a1_approved_at": "2026-09-02",
                    "a2_reviewer": "A2",
                    "a2_approved_at": "2026-09-02",
                    "a3_reviewer": "A3",
                    "a3_approved_at": "2026-09-02",
                    "references": "",
                    "notes": "",
                }
            )
            ...
            w.writerow(
                {
                    ...
                    "release_status": "Released",
                    "release_version": "1.0",
                    "a1_reviewer": "Mona Khalil",
                    "a1_approved_at": "2026-09-02 10:00:00",
                    "a2_reviewer": "Hesham Farouk",
                    "a2_approved_at": "2026-09-02 10:30:00",
                    "a3_reviewer": "Nadia Mostafa",
                    "a3_approved_at": "2026-09-02 11:00:00",
                    "references": "test",
                    "notes": "",
                }
            )
    ...
    def test_b15_catalog_preserves_po_while_updating_display(self):
        ...
        catalog = self._insert_catalog(
            src, app="construction", po_val="upstream po", translated="upstream po"
        )
        catalog.reload()
    ...
    def test_b06_drift_detection(self):
        ...
        try:
            ...
            self.assertTrue(
                any(src in d for d in h2["drift_details"]),
    ...
    def test_b17_hook_fail_closed(self):
        ...
        try:
            with tempfile.NamedTemporaryFile(
                ...
                tmp = f.name
            pathlib.Path(orig_path).write_text(
                pathlib.Path(tmp).read_text(encoding="utf-8"), encoding="utf-8"
            )
