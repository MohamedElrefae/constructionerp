...
def test_migration():
	...
	for theme_name in expected_themes:
		exists = frappe.db.sql("SELECT 1 FROM `tabConstruction Theme` WHERE name = %s LIMIT 1", (theme_name,))
...
def test_theme_resolution():
	...
	exists = frappe.db.sql("SELECT 1 FROM `tabConstruction Theme` WHERE name = %s LIMIT 1", (theme_name,))
	assert (
		len(exists) > 0 or theme_name == "Standard"
	), f"Resolved theme {theme_name} should exist or be 'Standard'"
