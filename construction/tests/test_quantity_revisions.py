...
class TestQuantityRevisions(FrappeTestCase):
    ...
    def _make_vo(self, header_name, item_name, revised_qty=None, revised_rate=None, line_type="Quantity Change"):
    ...
    def test_locking_boq_creates_baseline_revisions(self):
        ...
        revisions = frappe.get_all("BOQ Quantity Revision", 
    ...
    def test_current_revised_qty_updates_after_approved_revision(self):
        ...
        self.assertEqual(item.last_quantity_revision, 
            frappe.db.get_value("BOQ Quantity Revision", {"boq_item": item.name, "revision_type": "Increase Within 25%"}, "name"))
    ...
    def test_quantity_increase_computes_correct_delta_and_value(self):
        ...
        revision = frappe.get_doc("BOQ Quantity Revision", 
            {"boq_item": item.name, "revision_type": "Increase Above 25%"})
    ...
    def test_quantity_decrease_computes_correct_delta_and_value(self):
        ...
        revision = frappe.get_doc("BOQ Quantity Revision", 
            {"boq_item": item.name, "revision_type": "Decrease Within 25%"})
    ...
    def test_omission_sets_revised_qty_to_zero(self):
        ...
        revision = frappe.get_doc("BOQ Quantity Revision", 
            {"boq_item": item.name, "revision_type": "Omission"})
    ...
    def test_omitted_item_hidden_from_transaction_selectors(self):
        ...
        items = get_boq_items("BOQ Item", "", "name", 0, 10, {
            "boq_header": header.name,
        ...
        items_all = get_boq_items("BOQ Item", "", "name", 0, 10, {
            "boq_header": header.name,
        })
    ...
    def test_revision_type_auto_computed_correctly(self):
        ...
        revision1 = frappe.get_doc("BOQ Quantity Revision", 
        ...
        revision2 = frappe.get_doc("BOQ Quantity Revision", 
            {"boq_item": item.name, "revision_type": "Increase Above 25%"})
...
class TestQuantityRevisionService(FrappeTestCase):
    ...
    def test_create_lock_baseline_idempotent(self):
        ...
        count_before = frappe.db.count("BOQ Quantity Revision", {
            "boq_header": header.name,
        ...
        count_after = frappe.db.count("BOQ Quantity Revision", {
            "boq_header": header.name,
            "revision_type": "Original Lock"
        })
