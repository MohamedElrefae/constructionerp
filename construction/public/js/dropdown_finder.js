...
(function(){
  ...
  visibleULs.forEach(function(el, i){
    ...
    console.log(i + ':', el.tagName, 'class=' + (el.className || 'none'),
  ...
  menus.forEach(function(el, i){
    console.log(i + ':', el.className, 'parent:', el.parentElement?.className?.substring(0,40));
  });
  ...
  document.querySelectorAll('body *').forEach(function(el){
    ...
    if (s.display !== 'none' && s.position === 'absolute' && el.children.length > 2) {
      ...
      if (items.length > 0) {
        console.log(el.tagName, 'class=' + (el.className || 'none'),
          'items=' + items.length,
