...
(function () {
	...
	var ModernThemeLoader = {
		...
		fixSidebarColors: function() {
			...
			// Find all sidebar links and force white color
			var links = document.querySelectorAll('.layout-side-section a, .desk-sidebar a, [class*="sidebar"] a');
			...
			links.forEach(function(link, i) {
				...
				// Force white color if it's dark
				if (computed.color.includes('0, 0, 0') || computed.color.includes('rgb(0')) {
					console.log('[Theme Diagnostic] Fixing dark color on link', i);
					link.style.setProperty('color', '#e8f5e9', 'important');
				}
			});
		},
		...
		injectDynamicCSS: function (css, themeName) {
			...
			el.textContent = css;
			// Insert at END of head so API CSS naturally overrides inline CSS per spec
			document.head.appendChild(el);
			// Remove inline CSS after 500ms to avoid specificity wars per spec 2.3
			setTimeout(function() {
