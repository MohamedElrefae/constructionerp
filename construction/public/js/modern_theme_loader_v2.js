...
(function () {
    ...
    applyTheme: function (forcedMode) {
      ...
      frappe.call({
        ...
        callback: function (r) {
          ...
          if (forcedMode && mode !== forcedMode) { mode = forcedMode; source = 'dom_override'; }
          else if (modeToSend && mode !== modeToSend) { mode = modeToSend; source = 'request_override'; }
    ...
    fetchAndApplyCSSVariables: function (themeName) {
      ...
      frappe.call({
        ...
        error: function () {
          ...
          console.log('[Modern Theme] Dynamic CSS failed for:', themeName, '- using inline fallback');
        }
      });
    },
      ...
      frappe.call({
        ...
        callback: function (r) {
          if (r.message && r.message.css) {
            ...
            console.log('[Modern Theme] Dynamic CSS loaded for:', themeName, '(' + r.message.css.length + ' bytes)');
          }
        },
        error: function () {
  ...
  if (frappe.ui && frappe.ui.ThemeSwitcher) {
    frappe.ui.ThemeSwitcher = class CustomThemeSwitcher extends frappe.ui.ThemeSwitcher {
        ...
        return new Promise(function (resolve) {
          ...
          frappe.call({
            ...
            callback: function (r) {
              ...
              if (r.message && r.message.success && r.message.themes && r.message.themes.length > 0) {
                ...
                _cachedThemes = r.message.themes
                  .filter(function (t) {
                    ...
                    return t.name && t.name.toLowerCase().indexOf('construction') !== -1;
                  })
                  .map(function (t) {
                ...
                _cachedThemes = [
                  { name: 'light', label: '☀️ Light', info: 'Frappe Light Theme' },
                  { name: 'dark', label: '🌙 Dark', info: 'Frappe Dark Theme' },
                  { name: 'construction_light', label: '🏗️ Construction Light', info: 'Green-accented light theme' },
                  { name: 'construction_dark', label: '🏗️ Construction Dark', info: 'Green-accented dark theme' },
                  { name: 'automatic', label: '⚡ Automatic', info: 'Follows system preference' }
                ];
              }
              this.themes = _cachedThemes;
      ...
      switch_theme(theme_name) {
        ...
        if (themeLower === 'dark' || themeLower.indexOf('dark') !== -1) targetMode = 'dark';
        else if (themeLower === 'light' || themeLower.indexOf('light') !== -1) targetMode = 'light';
        ...
        try { localStorage.setItem('construction_theme', isConstructionTheme ? _norm(theme_name) : ''); } catch (e) { /* non-critical */ }

        this.hide();
      }

      render() {
        try { super.render(); } catch (e) { /* non-critical */ }
        setTimeout(function () { this._applyVisualIndicators(); }.bind(this), 150);
      }
      ...
      _applyVisualIndicators() {
        ...
        this.$wrapper.find('.theme-item').each(function () {
          ...
          if (t === sel) {
            $i.addClass('active-theme').css({ border: '2px solid #2E7D32', background: 'rgba(46,125,50,.1)' });
            $i.find('.theme-label').prepend('<span class="selected-indicator" style="color:#2E7D32;font-weight:bold;margin-right:8px;">\u2713 </span>');
          }
        });
      }
    };
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function () { ModernThemeLoader.init(); });
  } else {
    setTimeout(function () { ModernThemeLoader.init(); }, 0);
  }
})();
