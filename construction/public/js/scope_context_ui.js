(function () {
	...
	function populateDropdown(id, items, valueField, labelField, placeholder, currentValue) {
		...
		$menu.append(
			'<a class="dropdown-item" href="#" data-value="">' + (placeholder || "\u2014") + "</a>"
		);
		...
		if (dims.project) {
			...
			if (cur.cost_center) {
				...
				filteredProjects = (h.projects || []).filter(function (p) {
					return (
						p.company === cur.company && descendantCCs.indexOf(p.cost_center) !== -1
					);
				});
			} else {
		...
		if (dims.department) {
			...
			if (cur.cost_center) {
				...
				filteredDepts = (h.departments || []).filter(function (d) {
					return (
						d.company === cur.company && descendantCCsD.indexOf(d.cost_center) !== -1
					);
				});
			} else {
