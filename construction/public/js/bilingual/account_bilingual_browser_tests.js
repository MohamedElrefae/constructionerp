...
(function () {
  ...
  check("arabic label uses arabic name without internal name", function () {
    const out = label_for(
      { account_name_ar: "عربي", account_name: "English Name", value: "1100 - English Name - TC" },
      "ar"
    );
    return out === "عربي" && out.indexOf("1100") === -1 && out.indexOf("English Name") === -1;
  });

  check("arabic fallback to english, then identity — still no internal-name append", function () {
  ...
  check("english session prefers english then arabic", function () {
    const a = label_for({ account_name_ar: "عربي", account_name: "English", value: "x" }, "en");
  ...
  check("identity section renders on the open Account form", function () {
    ...
    const $w = frm.fields_dict.ct_bilingual_identity && frm.fields_dict.ct_bilingual_identity.$wrapper;
    if (!$w) return false;
    return (
