...
class ConstructionExportMenu {
    ...
    render() {
        ...
        var $group = $('<div class="btn-group construction-export-menu" style="margin-left:8px;"></div>');

        // Main button
        var $btn = $(
            '<button class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'
            + '<span class="hidden-xs">'
            + '<i class="fa fa-download" style="margin-right:4px;"></i> ' + __('Export')
            + '</span>'
        ...
        var $menu = $('<ul class="dropdown-menu dropdown-menu-right" style="min-width:220px;"></ul>');
        ...
        this.items.forEach(function (item) {
            ...
            var icon_html = item.icon ? '<i class="' + item.icon + '" style="width:18px;margin-right:6px;text-align:center;"></i>' : '';
            var $li = $('<li><a href="#" style="display:flex;align-items:center;padding:6px 16px;">'
                + icon_html + item.label + '</a></li>');
...
class ConstructionViewMenu {
    ...
    render() {
        ...
        var self = this;
        var current_item = this.items.find(function (i) { return i.value === self.current_view; });
        var current_icon = current_item ? current_item.icon : 'fa fa-eye';
        var current_label = current_item ? current_item.label : __('View');

        var $group = $('<div class="btn-group construction-view-menu" style="margin-left:8px;"></div>');

        var $btn = $(
            '<button class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'
            + '<span class="hidden-xs">'
            + '<i class="' + current_icon + '" style="margin-right:4px;"></i> ' + current_label
            + '</span>'
            + '<span class="visible-xs"><i class="' + current_icon + '"></i></span>'
            + ' <span class="caret"></span>'
            + '</button>'
        );

        var $menu = $('<ul class="dropdown-menu dropdown-menu-right" style="min-width:180px;"></ul>');

        this.items.forEach(function (item) {
            var icon_html = item.icon ? '<i class="' + item.icon + '" style="width:18px;margin-right:6px;text-align:center;"></i>' : '';
            var active_class = item.value === self.current_view ? ' style="display:flex;align-items:center;padding:6px 16px;font-weight:bold;background:#f0f4f7;"' : ' style="display:flex;align-items:center;padding:6px 16px;"';
            var check_html = item.value === self.current_view ? '<i class="fa fa-check" style="margin-left:auto;color:#36b37e;"></i>' : '';
            var $li = $('<li><a href="#"' + active_class + '>'
                + icon_html + item.label + check_html + '</a></li>');
            $li.find('a').on('click', function (e) {
                e.preventDefault();
...
if (typeof module !== 'undefined') {
    module.exports = { ConstructionExportMenu: ConstructionExportMenu, ConstructionViewMenu: ConstructionViewMenu };
}
