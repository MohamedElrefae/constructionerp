...
(function() {
  ...
  const themeCss = [...document.styleSheets].find(s =>
    s.href && s.href.includes('modern_theme.css')
  );
  ...
  awesompletes.forEach((el, i) => {
    ...
    console.log(`  Awesomplete #${i}:`, el.tagName,
      '\n    class:', el.className,
      '\n    style attr:', el.getAttribute('style'),
      '\n    computed bg:', style.backgroundColor,
      '\n    computed border-left:', style.borderLeftColor,
      '\n    computed color:', style.color,
      '\n    computed display:', style.display,
      '\n    parent:', el.parentElement?.tagName, el.parentElement?.className?.substring(0, 50));
    ...
    if (items[0]) {
      ...
      console.log(`    First item computed bg:`, itemStyle.backgroundColor,
        'color:', itemStyle.color);
    }
  });

  // Bootstrap dropdowns
  const dropdownMenus = document.querySelectorAll('.dropdown-menu.show, .dropdown.open .dropdown-menu, .dropdown-menu');
  console.log('\n  .dropdown-menu found:', dropdownMenus.length);
  dropdownMenus.forEach((el, i) => {
    const style = getComputedStyle(el);
    console.log(`  Dropdown #${i}:`, el.tagName,
      '\n    class:', el.className,
      '\n    style attr:', el.getAttribute('style'),
      '\n    computed bg:', style.backgroundColor,
      '\n    computed display:', style.display,
  ...
  selects.forEach((el, i) => {
    ...
    console.log(`  Select #${i}:`,
      ...
      '\n    computed bg:', style.backgroundColor,
      '\n    computed color:', style.color,
      '\n    computed border-left:', style.borderLeftWidth, style.borderLeftColor);
  });
  ...
  if (firstDropdown) {
    try {
      ...
      rules.slice(0, 5).forEach(r => {
        console.log('   ', r.selectorText.substring(0, 80),
