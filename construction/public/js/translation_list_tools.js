(function () {
	...
	function setupTranslationTools(listview) {
		if (!listview || listview.doctype !== "Translation" || listview.__ct_translation_tools_bound) {
			return;
		}
		...
		listview.page.add_menu_item(__("Missing Arabic (Prepare + Filter)"), () => {
			frappe.call({
				...
				return frappe.call({
					...
					frappe.show_alert({
						message: __("Prepared {0} rows. Missing Arabic now: {1}", [created, names.length]),
						indicator: "green",
					});
					listview.filter_area.clear().then(() => {
						addFilter(listview, ["Translation", "language", "=", "ar"]).then(() => {
							if (names.length) {
								addFilter(listview, ["Translation", "name", "in", names]);
							}
						});
					});
				});
