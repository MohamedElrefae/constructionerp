(function () {
  ...
  var ModernThemeLoader = {
    currentTheme: null, currentMode: null, version: VERSION,
    init: function () {
      console.log('[Modern Theme v' + this.version + '] Initializing...');
      if (typeof frappe === 'undefined') { setTimeout(function () { ModernThemeLoader.init(); }, 100); return; }
      try{var saved=localStorage.getItem('construction_theme');if(saved&&saved.indexOf('construction')===0){document.documentElement.setAttribute('data-modern-theme',saved);this.currentTheme=saved;}}catch(e){} this.applyTheme(); this.watchThemeChanges(); this.injectCSSVariables();
    },
    applyTheme: function (forcedMode) {
      ...
      frappe.call({
        ...
        callback: function (r) {
          if (!r.message) return;
          var tn = r.message.theme_name, m = r.message.mode, s = r.message.source;
          if (forcedMode && m !== forcedMode) { m = forcedMode; s = 'dom_override'; }
          else if (mode && m !== mode) { m = mode; s = 'request_override'; }
          self.currentTheme = tn; self.currentMode = m;
          document.documentElement.setAttribute('data-theme', m);
          var ex = document.documentElement.getAttribute('data-modern-theme');
          if (tn !== 'Standard') document.documentElement.setAttribute('data-modern-theme', tn);
          else if (!ex) document.documentElement.removeAttribute('data-modern-theme');
          self.updateNavbarIndicator(m);
          window.dispatchEvent(new CustomEvent('modern-theme-applied', { detail: { theme_name: tn, mode: m, source: s } }));
        },
        error: function () {
          if (forcedMode) { document.documentElement.setAttribute('data-theme', forcedMode); self.currentMode = forcedMode; self.updateNavbarIndicator(forcedMode); }
        }
      });
    },
    watchThemeChanges: function () {
      var self = this;
      if (window.matchMedia) window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', function (e) { self.applyTheme(e.matches ? 'dark' : 'light'); });
      var obs = new MutationObserver(function (muts) {
        for (var i = 0; i < muts.length; i++) {
          if (muts[i].attributeName === 'data-theme') {
            var raw = document.documentElement.getAttribute('data-theme'), old = self.currentMode;
            ...
            if (raw && raw !== old) { self.persistModePreference(raw); self.applyTheme(raw); }
          }
        }
      });
    ...
    persistModePreference: function (mode) {
      frappe.call({ method: 'construction.api.theme_api.save_user_mode', args: { mode: mode },
        callback: function (r) { if (r.message && r.message.success) console.log('[Modern Theme] Persisted mode: ' + mode); },
        error: function () {} });
    },
    injectCSSVariables: function () { document.body.classList.add('modern-theme-loaded'); this.injectNavbarIndicator(); },
    injectNavbarIndicator: function () {
      ...
      var chk = setInterval(function () {
        ...
        if (nav && !document.getElementById('modern-theme-indicator')) {
          ...
          var label = isCon ? (isDark ? '\uD83C\uDFD7\uFE0F Construction Dark' : '\uD83C\uDFD7\uFE0F Construction Light') : (isDark ? '\uD83C\uDF19 Dark' : '\u2600\uFE0F Light');
          var dc = isCon ? '#4CAF50' : (isDark ? '#60a5fa' : '#fbbf24');
          var li = document.createElement('li'); li.id = 'modern-theme-indicator'; li.className = 'nav-item dropdown';
          li.innerHTML = '<a class="nav-link" href="#" style="display:flex;align-items:center;padding:8px 12px;border-radius:20px;background:var(--hover-bg,rgba(0,0,0,.05));margin:0 8px;" onclick="return false;"><span id="theme-mode-dot" style="width:10px;height:10px;border-radius:50%;background:'+dc+';box-shadow:0 0 8px '+dc+';margin-right:8px;transition:all .3s ease;"></span><span id="theme-mode-text" style="font-size:.85rem;font-weight:500;">'+label+'</span></a>';
          var um = nav.querySelector('.dropdown-navbar-user'); if (um) nav.insertBefore(li, um); else nav.appendChild(li);
          console.log('[Modern Theme] Navbar indicator injected');
          if (!document.getElementById('theme-change-announcer')) {
            var ann = document.createElement('div'); ann.id = 'theme-change-announcer';
            ann.setAttribute('aria-live', 'polite'); ann.setAttribute('role', 'status');
    ...
    updateNavbarIndicator: function (mode) {
      var dot = document.getElementById('theme-mode-dot'), text = document.getElementById('theme-mode-text');
      if (!dot || !text) return;
      var mt = document.documentElement.getAttribute('data-modern-theme');
      var isCon = mt && mt.indexOf('construction') === 0, isDark = mode === 'dark';
      var label, dc;
      if (isCon) { label = isDark ? '\uD83C\uDFD7\uFE0F Construction Dark' : '\uD83C\uDFD7\uFE0F Construction Light'; dc = '#4CAF50'; }
      else { label = isDark ? '\uD83C\uDF19 Dark' : '\u2600\uFE0F Light'; dc = isDark ? '#60a5fa' : '#fbbf24'; }
      dot.style.background = dc; dot.style.boxShadow = '0 0 8px ' + dc; text.textContent = label;
      var ann = document.getElementById('theme-change-announcer');
      if (ann) { ann.textContent = ''; setTimeout(function () { ann.textContent = 'Theme changed to ' + label; }, 100); }
    },
    getCurrentTheme: function () { return { theme: this.currentTheme, mode: this.currentMode }; },
    refresh: function () { this.applyTheme(); },
    injectDynamicCSS: function (v) { if (!v) return; var e = document.getElementById('modern-theme-dynamic'); if (e) e.remove(); e = document.createElement('style'); e.id = 'modern-theme-dynamic'; e.textContent = ':root{\n' + v + '\n}'; document.head.appendChild(e); },
    fetchAndApplyCSSVariables: function (n) { var s = this; frappe.call({ method: 'construction.api.theme_api.get_theme_css_variables', args: { theme_name: n }, callback: function (r) { if (r.message && r.message.css_variables) s.injectDynamicCSS(r.message.css_variables); } }); }
  };
  ...
  if (frappe.ui && frappe.ui.ThemeSwitcher) {
    frappe.ui.ThemeSwitcher = class extends frappe.ui.ThemeSwitcher {
      fetch_themes() { return new Promise(function (res) { this.themes = [
        { name: 'light', label: '\u2600\uFE0F Light', info: 'Frappe Light' },
        { name: 'dark', label: '\uD83C\uDF19 Dark', info: 'Frappe Dark' },
        { name: 'construction_light', label: '\uD83C\uDFD7\uFE0F Construction Light', info: 'Green light' },
        { name: 'construction_dark', label: '\uD83C\uDFD7\uFE0F Construction Dark', info: 'Green dark' },
        { name: 'automatic', label: '\u26A1 Automatic', info: 'System' }
      ]; res(this.themes); }.bind(this)); }
      switch_theme(t) {
        var m = 'light';
        if (t === 'dark' || t === 'construction_dark') m = 'dark';
        else if (t === 'automatic') m = (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) ? 'dark' : 'light';
        if (t.indexOf('construction') === 0) { document.documentElement.setAttribute('data-modern-theme', t); ModernThemeLoader.currentTheme = t; frappe.call({ method: 'construction.api.theme_api.set_user_theme', args: { theme: t, mode: m }, error: function () {} }); }
        else { document.documentElement.removeAttribute('data-modern-theme'); ModernThemeLoader.currentTheme = 'Standard'; }
        document.documentElement.setAttribute('data-theme', m);
        ModernThemeLoader.currentMode = m; ModernThemeLoader.persistModePreference(m); ModernThemeLoader.updateNavbarIndicator(m); try{localStorage.setItem('construction_theme',t.indexOf('construction')===0?t:'');}catch(e){} this.hide();
      }
      render() { try { super.render(); } catch (e) {} setTimeout(function () { this._applyVisualIndicators(); }.bind(this), 150); }
      _applyVisualIndicators() {
        var ct = ModernThemeLoader.currentTheme || 'Standard', cm = ModernThemeLoader.currentMode || 'light';
        ...
        this.$wrapper.find('.theme-item').each(function () {
          var $i = $(this), t = $i.data('theme');
          $i.removeClass('active-theme').css({ border: '', background: '' }).find('.selected-indicator').remove();
          if (t === sel) { $i.addClass('active-theme').css({ border: '2px solid #2E7D32', background: 'rgba(46,125,50,.1)' }); $i.find('.theme-label').prepend('<span class="selected-indicator" style="color:#2E7D32;font-weight:bold;margin-right:8px;">\u2713 </span>'); }
        });
      }
    };
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function () { ModernThemeLoader.init(); });
  else setTimeout(function () { ModernThemeLoader.init(); }, 0);
})();
