		...
		if (rows) {
			for (var r = 0; r < rows.length; r++) {
				...
				for (var c = 0; c < cols.length; c++) {
					...
					html +=
						"<td>" +
						(val !== undefined && val !== null ? this._escape(String(val)) : "") +
						"</td>";
				}
				html += "</tr>";
			}
...
class PrintSettingsDialog {
	...
	_build_body_html() {
		...
		html +=
			...
			this.config_manager.get_width_total() +
			"</span>%";
		html += "</div>";
	...
	_build_column_row_html(col) {
		...
		// Visibility checkbox
		html +=
			'<input type="checkbox" class="col-visible-check" data-field-key="' +
			col.field_key +
			'"' +
			checked +
		...
		// Width input
		html +=
			'<input type="number" class="col-width-input" data-field-key="' +
			col.field_key +
			'" ' +
			'value="' +
			col.width +
	...
	_update_preview() {
		if (this.preview_panel && this.config_manager) {
			this.preview_panel.render(this.config_manager.get_visible_columns(), this.sample_data);
		}
	}
				...
				.catch(function (err) {
					...
					frappe.msgprint({
						...
						message:
							err && err.message
								? err.message
								: __("An error occurred during export. Please try again."),
						indicator: "red",
					});
				});
		} catch (err) {
