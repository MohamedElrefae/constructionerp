...
(function () {
	...
	function getControlInstance(el) {
		...
		if ($control_wrapper.length) {
			...
			if (fieldname) {
				...
				if ($grid_row.length) {
					...
					if (
						grid_row_obj &&
						grid_row_obj.fields_dict &&
						grid_row_obj.fields_dict[fieldname]
					) {
				...
				if (
					...
					cur_dialog.fields_dict[fieldname]
				) {
				...
				if (
					window.cur_list &&
					cur_list.page &&
					cur_list.page.fields_dict &&
					cur_list.page.fields_dict[fieldname]
				) {
		...
		if (frappe.query_report && frappe.query_report.filters) {
			const fieldname =
				$el.attr("data-fieldname") || $control_wrapper.attr("data-fieldname");
	...
	function enhanceSelect(el) {
		...
		const $select = $(el);
		if (
			$select.hasClass("hasDatepicker") ||
			$select.attr("data-fieldtype") === "Date" ||
		...
		if (!$select.is(":visible") && !$select.closest(".page-form, .filters-form").length)
			return;
		...
		function syncLabel() {
			...
			const placeholder =
				field && field.df && field.df.label ? __(field.df.label) : __("Select…");
	...
	function scanAndEnhance() {
		...
		$(".frappe-control[data-fieldtype='Select'] select.form-control").each(function () {
			try {
				enhanceSelect(this);
			} catch (e) {
				/* silent */
		...
		$(".frappe-control[data-fieldtype='MultiSelectList'] .multiselect-list").each(function () {
			try {
