...
(function () {
	...
	console.log(
		"[ScopeContext Filter] loading, scopeContext=" +
			!!window.scopeContext +
			" enabled=" +
	...
	function getDescendantCCNames(ccName, costCenters) {
		if (!ccName) return [];
		var scopeCC = (costCenters || []).find(function (cc) {
	...
	if (frappe.views && frappe.views.ListView) {
		...
		frappe.views.ListView.prototype.get_args = function () {
			...
			if (dims.cost_center && scopeFilter.cost_center && hasField.cost_center) {
				...
				var descendantNames = getDescendantCCNames(
					scopeFilter.cost_center,
					h.cost_centers
				);
