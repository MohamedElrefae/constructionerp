...
def create_quantity_revision(boq_item, previous_qty, revised_qty, contract_unit_price, revised_unit_price, variation_order=None, reason=None, rate_change_justification=None, owner_refs=None, status="Draft"):
...
def apply_approved_revision(revision):
    ...
    frappe.db.sql("SELECT name FROM `tabBOQ Item` WHERE name = %(name)s FOR UPDATE", {"name": revision.boq_item})
    
    # Update current quantities
    frappe.db.set_value("BOQ Item", revision.boq_item, {
...
def create_variation_item_revision(boq_item, quantity, unit_price, variation_order, owner_refs=None, rate_change_justification=None):
...
def process_approved_vo_lines(vo):
    ...
    """
    from construction.construction.doctype.variation_order.variation_order import create_variation_structure_and_item
    ...
    for line in vo.lines:
        ...
        try:
            if line.line_type == "New Item":
                ...
                frappe.db.set_value("BOQ Item", item_name, {
                    "original_qty": 0,
