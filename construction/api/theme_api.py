    ...
    try:
        ...
        try:
            ...
            if user_theme_pref and not user_theme_pref.inherit_from_site:
                ...
                construction_theme_name = (
                    user_theme_pref.dark_theme 
                    if mode == "dark" 
                    else user_theme_pref.light_theme
                )
                ...
                if construction_theme_name:
                    ...
                    theme_exists = frappe.db.exists(
                        "Construction Theme", 
                        {"name": construction_theme_name, "is_active": 1}
                    )
                    
                    if theme_exists:
                        # Fetch feature toggles from theme
                        toggles = frappe.db.get_value(
                            "Construction Theme",
                            construction_theme_name,
                            [
                                "hide_help_button", "hide_search_bar", "hide_sidebar",
                                "hide_like_comment", "mobile_card_view"
                            ],
                            as_dict=True
                        ) or {}
        ...
        try:
            ...
            default_theme = frappe.db.get_value(
                ...
                {
                    "is_active": 1,
                    "is_default_light" if mode == "light" else "is_default_dark": 1
                },
                "name"
            )
        ...
        try:
            ...
            default_theme_name = (
                site_settings.default_dark_theme if mode == "dark"
                else site_settings.default_light_theme
            )
            if default_theme_name:
                theme_exists = frappe.db.exists(
                    "Construction Theme",
                    {"name": default_theme_name, "is_active": 1}
                )
...
def preview_theme(theme_name=None, temporary=False, **color_fields):
    ...
    try:
        if temporary and color_fields:
            ...
            return {
                "css": "",
                "error": "Either theme_name or temporary=True with color_fields required"
            }
