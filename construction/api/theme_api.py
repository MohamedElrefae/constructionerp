...
def get_effective_desk_theme():
	...
	try:
		...
		try:
			...
			return {"theme_name": "Standard", "mode": mode, "source": "frappe_default"}
		...
		if allow_user_override:
			try:
				...
				if user_theme and not user_theme.inherit_from_site:
					...
					if theme_name:
						return {"theme_name": theme_name, "mode": mode, "source": "user_override"}
			except frappe.DoesNotExistError:
				pass  # No user override set

		# Fall back to site defaults
		theme_name = (
			site_settings.default_light_theme if mode == "light" else site_settings.default_dark_theme
		)

		if theme_name:
			return {"theme_name": theme_name, "mode": mode, "source": "site_default"}
		...
		return {"theme_name": "Standard", "mode": "light", "source": "error_fallback"}
...
def save_modern_theme_settings(
	default_light_theme, default_dark_theme, allow_user_override=True, force_modern_ui=True
):
	"""
