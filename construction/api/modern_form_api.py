            ...
            if field.fieldtype in ["Select", "Link", "Table MultiSelect"]:
                if field.fieldtype == "Select" and field.options:
                    field_config["options"] = [
                        {"value": opt, "label": opt} for opt in field.options.split("\n")
                    ]
...
def update_document(doctype, name, data):
    ...
    try:
        ...
        if not doc.has_permission("write"):
            return {"success": False, "error": _("Not permitted to update this document")}
...
def delete_document(doctype, name):
    ...
    try:
        ...
        return {"success": True, "message": _("{0} '{1}' deleted successfully").format(doctype, name)}
...
def validate_field(doctype, fieldname, value, context=None):
    ...
    try:
        ...
        if not field:
            return {"valid": False, "error": f"Field '{fieldname}' not found"}
...
def search_link(doctype, txt, filters=None, page_length=20):
    ...
    try:
        ...
        return {
            "success": True,
            "results": [{"value": r.name, "label": r.get("title") or r.name} for r in results],
        }
