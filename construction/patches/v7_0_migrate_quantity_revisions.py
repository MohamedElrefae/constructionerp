...
def _migrate_boq_items():
    ...
    items = frappe.get_all("BOQ Item", fields=["name", "quantity", "is_variation_item", "contract_unit_price"])
    ...
    for item in items:
        ...
        frappe.db.set_value("BOQ Item", item.name, {
    ...
    for header in headers:
        try:
            ...
            if result.get("success"):
                frappe.log_error(f"Created baseline for {header}: {result.get('created', 0)} revisions", "Migration v7.0")
...
def _migrate_variation_items():
    ...
    for item in variation_items:
        ...
        existing = frappe.db.get_value("BOQ Quantity Revision", {
            "boq_item": item.name,
        ...
        try:
            ...
            # Link to BOQ Item
            frappe.db.set_value("BOQ Item", item.name, "last_quantity_revision", revision.name, update_modified=False)
        except Exception as e:
...
def _migrate_legacy_vo_lines():
    ...
    legacy_lines = frappe.get_all(
        ...
        filters={
            "created_boq_item": ["!=", ""],
            "created_quantity_revision": ["", None]
        },
    ...
    for line in legacy_lines:
        ...
        revision = frappe.db.get_value("BOQ Quantity Revision", {
            "boq_item": line.created_boq_item,
            "revision_type": "New Variation Item"
        }, "name")
        
        if revision:
            frappe.db.set_value("VO Line", line.name, "created_quantity_revision", revision, update_modified=False)
...
def _update_header_totals():
    ...
    for header in headers:
        try:
            ...
            frappe.db.set_value("BOQ Header", header.name, "total_revised_value", total_revised, update_modified=False)
        except Exception as e:
