...
def execute():
    ...
    themes = frappe.get_all(
        "Construction Theme",
        fields=["name", "accent_primary", "surface_bg", "text_primary",
                "text_secondary", "sidebar_bg", "body_bg", "border_color"],
    )
    ...
    for theme_data in themes:
        ...
        if not frappe.db.get_value("Construction Theme", theme_data.name, "primary_btn_hover_bg"):
            updates["primary_btn_hover_bg"] = (
                lighten_hex(btn_bg) if is_dark else darken_hex(btn_bg)
            )

        sec_bg = updates.get("secondary_btn_bg") or theme_data.surface_bg
        if sec_bg and not frappe.db.get_value("Construction Theme", theme_data.name, "secondary_btn_hover_bg"):
            updates["secondary_btn_hover_bg"] = (
                lighten_hex(sec_bg) if is_dark else darken_hex(sec_bg)
            )

        if updates:
            frappe.db.set_value(
                "Construction Theme", theme_data.name, updates,
