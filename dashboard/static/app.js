...
function updateProposalDisplay(reviewCtx) {
  ...
  if (proposal) {
    ...
    btn.style.display = (reviewCtx.plan_granted || reviewCtx.current_stage !== 'plan') ? 'none' : 'inline-block';
  } else {
...
function updateReviewDisplay(reviewCtx, stateData) {
  ...
  if (!gate || gate.scope !== 'PLAN') {
    details.textContent = `Pending Gate: ${gate ? gate.scope : 'None'}\nNo pending PLAN approval gate.`;
    createBtn.style.display = 'none';
    approveBtn.style.display = 'none';
    return;
  }
...
function updateAiContextDisplay(aiCtx) {
  ...
  if (memEl) {
    memEl.textContent = (aiCtx && aiCtx.session_memory_md) ? aiCtx.session_memory_md : '(Session memory is empty)';
  }
...
async function refreshTaskDetail() {
  ...
  try {
    ...
    if (stateRes.ok && planRes.ok) {
      ...
      document.getElementById('detail-branch-badge').textContent = stateData.branch || 'Unknown Branch';
      document.getElementById('detail-status-badge').textContent = stateData.status || 'UNKNOWN';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  setupForm.addEventListener('submit', async (e) => {
    ...
    if (new_password === current_password) {
      errorDiv.textContent = 'New permanent password must be different from current password';
      errorDiv.style.display = 'block';
      return;
    }
    ...
    bootstrapForm.addEventListener('submit', async (e) => {
      ...
      try {
        const res = await apiFetch('/api/tasks/bootstrap', {
          method: 'POST',
          body: JSON.stringify({ work_item: workItem, user_brief: userBrief, base_ref: baseRef }),
        });
        const data = await res.json();
        if (res.ok && data.ok) {
          msgDiv.className = 'alert alert-success';
  ...
  if (adoptBtn) {
    adoptBtn.addEventListener('click', async () => {
      ...
      try {
        const res = await apiFetch(`/api/tasks/${encodeURIComponent(activeTaskId)}/adopt-scope`, {
          method: 'POST',
          body: JSON.stringify({
  ...
  if (createReviewBtn) {
    createReviewBtn.addEventListener('click', async () => {
      ...
      const planText = (currentPlanData && currentPlanData.plan_text) ? currentPlanData.plan_text : '';
      ...
      try {
        const res = await apiFetch(`/api/tasks/${encodeURIComponent(activeTaskId)}/create-review`, {
          method: 'POST',
          body: JSON.stringify({
  ...
  if (approveBtn) {
    approveBtn.addEventListener('click', async () => {
      ...
      try {
        const res = await apiFetch(`/api/tasks/${encodeURIComponent(activeTaskId)}/approve-plan`, {
          method: 'POST',
          body: JSON.stringify({}),
        });
        const data = await res.json();
